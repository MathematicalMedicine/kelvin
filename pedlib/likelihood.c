...
loop_child_multi_locus_genotype (Person * pPChild, Person * pProband,
				 HaplotypePair * pHaplo, int child,
...
compute_likelihood (PedigreeSet * pPedigreeList)
{
  ...
  for (i = 0; i <= 20; i++)
    {
      pPedigreeList->hetLR[i] = 1;
      pPedigreeList->log10HetLR[i] = 0;
    }
...
//                fprintf(stderr,"end of polynomial building :\n");
...
//                fprintf(stderr,"\n");
	...
	{
	  logMsg (LOGLIKELIHOOD, LOGWARNING,
		  "Pedigree %s with likelihood 0.\n", pPedigree->sPedigreeID);
  ...
  logMsg (LOGLIKELIHOOD, LOGDEBUG, "Sum of log Likelihood is: %e\n",
	  sum_log_likelihood);
...
compute_pedigree_likelihood (Pedigree * pPedigree)
{
  ...
  double likelihood;
#ifndef NO_POLYNOMIAL
  Polynomial *pLikelihoodPolynomial;
#endif
...
#else
  pPedigree->likelihood = likelihood;
  logMsg (LOGLIKELIHOOD, LOGDEBUG, "log Likelihood for pedigree %d is: %e\n",
...
peel_graph (NuclearFamily * pNucFam, Person * pProband, int peelingDirection)
{
  ...
  logMsg (LOGPARENTALPAIR, LOGDEBUG, "\t Proband (%s) haplotype: \n",
	  pProband->sID);
		       ...
		       void *penetrance, void *weight)
{
  ...
  Locus *pLocus = originalLocusList.ppLocusList[locusList.pLocusIndex[locus]];
...
#ifndef NO_POLYNOMIAL
  Polynomial *newPenetrancePolynomial;
  Polynomial *newWeightPolynomial;
#endif
      ...
      else
	{
	  ...
	  if (modelOptions.polynomial == TRUE)
		...
		timesExp (2,
			  pProband->pLikelihood[multiLocusIndex].
			  likelihoodPolynomial, 1,
			  pNucFam->likelihoodPolynomial, 1);
				   ...
				   Person * pProband, int peelingDirection)
{
  ...
  for (locus = 0; locus < originalLocusList.numLocus; locus++)
    {
      ...
      numHaplotypePair *= pNucFam->pNumParentalPair[locus];
    }
  pNucFam->pHaplotypePairs =
    (HaplotypePair *) MALLOC ("PucFam->pHaplotypePairs",
			      sizeof (HaplotypePair) * numHaplotypePair);
  memset (pNucFam->pHaplotypePairs, 0,
	  sizeof (HaplotypePair) * numHaplotypePair);

  haplo.ppParentalPairs = (ParentalPair **) MALLOC ("haplo.ppParentalPairs",
						    sizeof (ParentalPair *) *
						    locusList.numLocus);
  memset (haplo.ppParentalPairs, 0,
	  sizeof (ParentalPair *) * locusList.numLocus);
  ...
  logMsg (LOGPARENTALPAIR, LOGDEBUG, "Haplotype for nuclear family No. %d:\n",
	  pNucFam->nuclearFamilyIndex);
  logMsg (LOGPARENTALPAIR, LOGDEBUG, "\t\t\t DAD(%s)\t\t\t MOM(%s)\n",
	  pNucFam->pParents[DAD]->sID, pNucFam->pParents[MOM]->sID);
#ifndef NO_POLYNOMIAL
  if (modelOptions.polynomial == TRUE)
    {
      loop_parental_pair (pNucFam, pProband, 0, &haplo, multiLocusIndex,
			  (void *) weightPolynomial,
...
//            pNucFam->nuclearFamilyIndex, evaluateValue(sumPolynomial));
    ...
    {
      loop_parental_pair (pNucFam, pProband, 0, &haplo, multiLocusIndex,
			  (void *) weight, (void *) penetrance, &sum);
      logMsg (LOGLIKELIHOOD, LOGDEBUG,
		    ...
		    void *dWeight[2], void *dPenetrance[2], void *pParentSum)
{
  ...
  Polynomial *freqPolynomial;
  Polynomial *sumPolynomial;
  Polynomial *probPolynomial;
  Polynomial *newWeightPolynomial[2];
  Polynomial *newPenetrancePolynomial[2];
  Polynomial *childProductPolynomial;
  Polynomial *childPenetrancePolynomial;
  if (modelOptions.polynomial == TRUE)
    {
  ...
  while (pPair != NULL)
		  ...
		  if (locus > 0)
		    {
		      ...
		      pTempPair = pHaplo->ppParentalPairs[locus - 1];
		      for (j = DAD; j <= MOM; j++)
			{
      ...
      else
	{
	  ...
	  for (child = 0; child < pNucFam->numChildren; child++)
	    {
	      prevInformativeLocus[DAD] = -1;
	      prevInformativeLocus[MOM] = -1;
#ifndef NO_POLYNOMIAL
	      if (modelOptions.polynomial == TRUE)
		{
		  sumPolynomial = constantExp (0);
...
//          fprintf(stderr,"likelihood=%f\n",sum);
	  ...
	  *(double *) pParentSum += pHaplo->likelihood;
	  ...
	  for (k = 0; k < locusList.numLocus; k++)
	    {
	      pTempPair = pHaplo->ppParentalPairs[k];
...
//      logMsg (LOGPARENTALPAIR, LOGDEBUG, "\t\t Likelihood = %e\n",
				 ...
				 int chromosome[2])
{
  int i;
  Genotype *pGeno;
  ParentalPair *pParentalPair = pHaplo->ppParentalPairs[locus];
  int parent;
  ...
  for (i = 0; i < pParentalPair->pChildGenoLen[child]; i++)
    {
      ...
      newInformativeLocus[DAD] = prevInformativeLocus[DAD];
      newInformativeLocus[MOM] = prevInformativeLocus[MOM];
#ifndef NO_POLYNOMIAL
      if (modelOptions.polynomial == TRUE)
	{
	  newPenetrancePolynomial =
	    timesExp (2, (Polynomial *) pPenetrance, 1,
		      pGeno->penetrancePolynomial, 1);
      ...
      if (locus < locusList.numLocus)
	{
	  ...
	  for (parent = DAD; parent <= MOM; parent++)
	    {
	      newChromosome[parent] = 0;
	      if (is_parent_child_allele_compatible (alleleSetLen,
						     pParentalPair->
						     pGenotype[parent]->
						     pAlleleBits[DAD], parent,
						     pGeno))
		/* mark that the allele from paternal chromosome of the parent 
		 * is transmitted */
		newChromosome[parent] += DAD + 1;
	      if (is_parent_child_allele_compatible (alleleSetLen,
						     pParentalPair->
						     pGenotype[parent]->
						     pAlleleBits[MOM], parent,
						     pGeno))
		/* mark that the allele from maternal chromosome of the parent 
		 * is transmitted */
		newChromosome[parent] += MOM + 1;

	      /* current locus is informative */
//        fprintf(stderr,"newChromosome[%d]=%d child genotype (%d  %d)\n",
//                        parent,newChromosome[parent],
//                        pGeno->allele[0],pGeno->allele[1]);
	      if (newChromosome[parent] != 3)
		{
		  /* parent is not homozygouos */
		  if (prevInformativeLocus[parent] == -1)
		    {
#ifndef NO_POLYNOMIAL
		      if (modelOptions.polynomial == TRUE)
			{
			  newProbPolynomial =
			    timesExp (2, newProbPolynomial, 1,
				      constantExp (0.5), 1);

//                fprintf(stderr," Polynomial 1 is: ");
//                expPrinting(newProbPolynomial);
//                fprintf(stderr,"\n value=%f\n", evaluateValue(newProbPolynomial));
			}
		      else
			{
			  /* first informative locus */
			  newProb *= 0.5;
			}
#else
		      newProb *= 0.5;
		    ...
		    {
		      /* check whether there is recombinant or not */
		      if (newChromosome[parent] == chromosome[parent])
			{
			  /* no recomninant */
			  if (locus == prevInformativeLocus[parent] + 1)
			    {
#ifndef NO_POLYNOMIAL
			      if (modelOptions.polynomial == TRUE)
				{
				  char vName[100];
				  sprintf (vName, "theta[%d]", locus);
				  newProbPolynomial =
				    timesExp (2, newProbPolynomial, 1,
					      plusExp (2, 1.0,
						       constantExp (1), -1.0,
							...
							pPrevLocusDistance
							[locus], vName)), 1);
//                  fprintf(stderr,"Polynomial 2 is: ");
//                  expPrinting(newProbPolynomial);
//                  fprintf(stderr,"\n value=%f\n", evaluateValue(newProbPolynomial));
				}
			      else
				{
				  newProb *=
				    (1 - locusList.pPrevLocusDistance[locus]);
				}
#else
			      newProb *=
				(1 - locusList.pPrevLocusDistance[locus]);
			    ...
			    {
			      /* no recombinants at both informative loci or 
			       * double recombinants */
#ifndef NO_POLYNOMIAL
			      if (modelOptions.polynomial == TRUE)
				{
				  char vName1[100], vName2[100];
				  sprintf (vName1, "theta[%d]", locus);
				  sprintf (vName2, "theta2[%d]",
					   prevInformativeLocus[parent]);
				  newProbPolynomial = timesExp (2, newProbPolynomial, 1, plusExp (2, 1.0, timesExp (2, plusExp (2, 1.0, constantExp (1), -1.0, variableExp (&locusList.pPrevLocusDistance[locus], vName1)),	//endof plusExp
														    1, plusExp (2, 1.0, constantExp (1), -1.0, variableExp (&locusList.pNextLocusDistance[prevInformativeLocus[parent]], vName2)),	//end of plusExp
														    1),	//end of timesExp
												  1.0, timesExp (2, variableExp (&locusList.pPrevLocusDistance[locus], vName1), 1, variableExp (&locusList.pNextLocusDistance[prevInformativeLocus[parent]], vName2), 1)	//end of timesExp
								),	//end of plus exp
								1);	//end of timesExp
//                  fprintf(stderr,"Polynomial 3 is: ");
//                  expPrinting(newProbPolynomial);
//                  fprintf(stderr,"\n value=%f\n", evaluateValue(newProbPolynomial));
				}
			      else
				{
				  newProb *=
				    ((1 -
				      locusList.pPrevLocusDistance[locus]) *
				     (1 -
				      locusList.
				      pNextLocusDistance[prevInformativeLocus
							 [parent]]) +
				     locusList.pPrevLocusDistance[locus] *
				     locusList.
				     pNextLocusDistance[prevInformativeLocus
			      ...
			      newProb *=
				((1 -
				  locusList.pPrevLocusDistance[locus]) * (1 -
									  locusList.
									  pNextLocusDistance
									  [prevInformativeLocus
									   [parent]])
		      ...
		      else
			{
			  ...
			  if (locus == prevInformativeLocus[parent] + 1)
			      ...
			      if (modelOptions.polynomial == TRUE)
				{
				  ...
				  sprintf (vName, "theta[%d]", locus);
				  newProbPolynomial =
				    timesExp (2, newProbPolynomial,
					      1,
					      variableExp (&locusList.
							   pPrevLocusDistance
							   [locus], vName),
					      1);
//                  fprintf(stderr,"Polynomial 4 is: ");
//                  expPrinting(newProbPolynomial);
//                  fprintf(stderr,"\n value=%f\n", evaluateValue(newProbPolynomial));
				}
			      else
			      ...
			      if (modelOptions.polynomial == TRUE)
				{
				  ...
				  Polynomial *tempP1, *tempP2, *tempP3,
				    *tempP4, *tempP5, *tempP6, *tempP7;
				  tempP4 =
				    variableExp (&locusList.
						 pPrevLocusDistance[locus],
						 vName1);
				  tempP2 =
				    variableExp (&locusList.
						 pNextLocusDistance
				  ...
				  tempP3 =
				    plusExp (2, 1.0, constantExp (1), -1.0,
					     tempP2);
				  tempP5 = timesExp (2, tempP1, 1, tempP2, 1);
				  tempP6 = timesExp (2, tempP3, 1, tempP4, 1);
				  tempP7 =
				    plusExp (2, 1.0, tempP5, 1.0, tempP6);
				  newProbPolynomial =
				    timesExp (2, newProbPolynomial, 1, tempP7,
					      1);
				  fprintf (stderr,
					   "BBBBBBBBBBBBBBBBBBBBBB\b");
//                  fprintf(stderr,"Polynomial 5 is: ");
//                  expPrinting(newProbPolynomial);
//                  fprintf(stderr,"\n value=%f\n", evaluateValue(newProbPolynomial));
				...
				{
				  /* only one recombinants at both informative loci */
				  newProb *=
				    (1 -
				     locusList.pPrevLocusDistance[locus]) *
				    locusList.
		    ...
		    }		/* there is a previous locus that's informative */
		  newChromosome2[parent] = newChromosome[parent];
		  newInformativeLocus[parent] = locus;
		}		/* current locus is informative */
	  ...
	  if (locus < locusList.numLocus - 1)
	      ...
	      if (modelOptions.polynomial == TRUE)
		{
		  if (pChild == pProband)
		    ...
		    *(Polynomial **) pSum = plusExp (2, 1.0, *(Polynomial **) pSum, 1.0, timesExp (2, newProbPolynomial, 1, pChild->pLikelihood[multiLocusIndex].likelihoodPolynomial, 1)	//end of timesExp
		      );	//end of plusExp
		}
	      else
		{
