				   ...
				   Person * pProband, int peelingDirection)
{
  ...
  if (modelOptions.polynomial == TRUE)
    {
      loop_parental_pair (pNucFam, pProband, 0, &parentalPairSpace, multiLocusIndex,
		    ...
		    void *dWeight[2], void *dPenetrance[2], void *pParentSum)
{
  ...
  //  pPair = pNucFam->ppParentalPair[locus];
  numPair = 0;
  while (numPair < pHaplo->pNumParentalPair[locus])
    {
		  ...
		  if (locus > 0)
		    {
		      ...
		      pTempPair = pHaplo->ppParentalPair[locus - 1];
		      for (j = DAD; j <= MOM; j++)
			{
...
//          fprintf(stderr,"likelihood=%f\n",sum);
	  ...
	  *(double *) pParentSum += pHaplo->likelihood;
	  ...
	  for (k = 0; k < locusList.numLocus; k++)
	    {
	      pTempPair = &pHaplo->ppParentalPair[k][pHaplo->pParentalPairInd[k]];
				 ...
				 int chromosome[2])
{
  int i;
  Genotype *pGeno;
  ParentalPair *pParentalPair = &pHaplo->ppParentalPair[locus][pHaplo->pParentalPairInd[locus]];
  int parent;
...
//                  fprintf(stderr,"\n value=%f\n", evaluateValue(newProbPolynomial));
			      ...
			      if (modelOptions.polynomial == TRUE)
				{
				  ...
				  newProbPolynomial = timesExp (2, newProbPolynomial, 1, plusExp (2, 1.0, timesExp (2, plusExp (2, 1.0, constantExp (1), -1.0, variableExp (&locusList.pPrevLocusDistance[locus], NULL, 'D',vName1)),	//endof plusExp
														    1, plusExp (2, 1.0, constantExp (1), -1.0, variableExp (&locusList.pNextLocusDistance[prevInformativeLocus[parent]], NULL,'D',vName2)),	 //end of plusExp
													    1),	//end of timesExp
												  1.0, timesExp (2, variableExp (&locusList.pPrevLocusDistance[locus], NULL,'D',vName1), 1, 
														    variableExp (&locusList.pNextLocusDistance[prevInformativeLocus[parent]], NULL,'D', vName2), 1)	//end of timesExp
								),	//end of plus exp
								1);	//end of timesExp
