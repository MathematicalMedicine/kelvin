...
//                                   locus,newPenetrance, *((double *)pSum),newProb);
	      ...
	      if (modelOptions.polynomial == TRUE)
		{
		  newProbPolynomial =
		    timesExp (2,
			      xmissionMatrix[newXmissionIndex[DAD]].probPoly[DAD+1],
			      1,
			      xmissionMatrix[newXmissionIndex[MOM]].probPoly[MOM+1],
			      1);
  ...
  for (pattern = 1; pattern <= 3; pattern++)
	      ...
	      if (lastHetPattern == pattern)
		{
		  ...
		  if (lastHetLoc == loc - 1)
		      ...
		      if (modelOptions.polynomial == TRUE)
			{
			  ...
			  *((double *) prob[i]) *
			  ((1 -
			    locusList->pNextLocusDistance[i][lastHetLoc]) * (1 -
									  locusList->
									  pPrevLocusDistance
									  [i][loc])
			   +
			   locusList->pNextLocusDistance[i][lastHetLoc] *
      ...
      else
	{
	  ...
	  if (modelOptions.polynomial == TRUE)
	    ...
	    populate_xmission_matrix (pMatrix, totalLoci,
				      (void *)newProbPtr, newCellIndex,
				      newLastHetLoc,
				      newLastHetPattern, newNumHet, loc + 1);
#else
	  populate_xmission_matrix (pMatrix, totalLoci,
				    newProbPtr, newCellIndex,
				    newLastHetLoc,
				    newLastHetPattern, newNumHet, loc + 1);
#endif
	}
