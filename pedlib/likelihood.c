...
int loop_child_multi_locus_genotype ();
...
void
loop_phases (int locus, int multiLocusIndex[2],
 ...
 * likelihoods numLocus - number of loci we analyze at a time
 */
int
allocate_likelihood_space (PedigreeSet * pPedigreeList, int numLocus1)
{
  ...
  powOf3 = (int *) malloc (numLocus * sizeof (int));
  powOf3[0] = 1;
  for (i = 1; i < numLocus; i++)
    {
      powOf3[i] = powOf3[i - 1] * 3;
    }

  /* likelihood work space */
  genoIndex = (int *) malloc (sizeof (int) * numLocus);
  genoPos = (int *) malloc (sizeof (int) * numLocus);
  chromosome[DAD] = (int *) malloc (sizeof (int) * numLocus);
  chromosome[MOM] = (int *) malloc (sizeof (int) * numLocus);
	  ...
	  if (pPedigree->likelihoodPolynomial == NULL)
	    {
	      ...
	      /*
	       * put a stamp in the polynomial list to mark
	       * the beginning of likelihood build for this
	       * pedigree
	       */
	      ...
	      /*
	       * clean up polynomials that are not used in
	       * the final pedigree likelihood
	       */
      ...
      for (i = 0; i < pProband->numTmpLikelihood; i++)
	{
	  ...
	  if (modelOptions.polynomial == TRUE)
		...
		timesExp (2,
			  ...
			  pConditional->tmpslot.tmpLikelihoodPolynomial, 1,
			  0);
  ...
  while (pGenotype != NULL)
      ...
      else
	{
	   ...
	   */
	    ...
	    {			/* NOT first time updating the likelihood for
...
loop_parental_pair (int locus, int multiLocusIndex[2], void *dWeight[2])
{
  ...
  while ((numPair + 1) < pHaplo->pNumParentalPair[locus])
	...
	{			/* got a complete set of parental pairs */
	  ...
	  if (pNucFam->totalRelatedPPair[locus] == 1)
	  ...
	  else if (pNucFam->totalRelatedPPair[locus] > 0)
	    {
	      ...
	      pNucFam->numHetLocus[spouse] =
		pNucFam->tmpNumHet[spouse][locus];
	      /*
	       * set bit mask - all bits set to 1 - number
	       * of bits == number of het loci
	       */
	      pNucFam->hetLocusBits[head] =
		bitMask[pNucFam->numHetLocus[head]];
...
#ifndef NO_POLYNOMIAL
	      if (modelOptions.polynomial == TRUE)
		loop_phases (0, multiLocusIndex2, multiLocusPhase,
			     (void *) newWeightPolynomial);
	      else
		loop_phases (0, multiLocusIndex2, multiLocusPhase,
			     (void *) newWeight);
	      ...
	      if (pNucFam->childProbandFlag == TRUE)
		{
		  ...
		  for (j = 0; j <= bitMask[pNucFam->numHetLocus[head]]; j++)
		    {
		      for (k = 0; k <= bitMask[pNucFam->numHetLocus[spouse]];
			   k++)
			{
	     ...
	     int multiLocusPhase[2], void *dWeight[2])
{
  ...
  while ((numPair += 1) < end)
      ...
      for (i = DAD; i <= MOM; i++)
	{
      ...
      if (locus < locusList->numLocus - 1)
	{
	  loop_phases (locus + 1, multiLocusIndex, multiLocusPhase, dWeight);
	}
      else
	{			/* got a complete multilocus parental pair */
		  ...
		  if (modelOptions.polynomial == TRUE)
		    {
		      ...
		      KLOG (LOGLIKELIHOOD, LOGDEBUG,
			    ...
			    likelihoodIndex,
			    evaluateValue (ppairMatrix[phase[proband]]
					   [phase[spouse]].slot.
					   likelihoodPolynomial));
#endif
		    }
	  ...
	  if (calculateFlag == 1)
	    {
	      calculate_likelihood (multiLocusIndex, multiLocusPhase,
				    dWeight);
...
int
calculate_likelihood (int multiLocusIndex[2],
		      int multiLocusPhase[2], void *dWeight[2])
{
  int i;
  int proband;
  ...
  for (i = DAD; i <= MOM; i++)
	...
	{			/* first time we work on this parent */
	  if (pNucFam->pParents[i]->pParents[DAD] == NULL)
	    {
	      /*
	      ...
	      if (modelOptions.equilibrium == LINKAGE_EQUILIBRIUM)
		{
	      ...
	      else if (pParent[i]->loopBreaker == 0)
		{		/* founder under LD */
#ifndef NO_POlYNOMIAL
		  if (modelOptions.polynomial == TRUE)
		    {
		      get_haplotype_freq (numLocus - 1, i,
					  &newWeightPolynomial[i]);
		    }
		  else
		    {
...
loop_child_multi_locus_genotype ()
{
  ...
  /* initialize genoIndex for each locus to be 0 */
  memset (genoIndex, 0, sizeof (int) * numLocus);
  multiLocusIndex = 0;
  xmissionIndex[DAD] = 0;
  xmissionIndex[MOM] = 0;
  for (l = 0; l < numLocus; l++)
    {
      /* get the parental pair at this locus */
      pParentalPair = &pHaplo->ppParentalPair[l][pHaplo->pParentalPairInd[l]];
      pHaplo->pChildGenoInd[l] = 0;
      /* get the position of the first compatible genotype for this child */
      pGenotype = pParentalPair->pppChildGenoList[child][0];
      KLOG (LOGLIKELIHOOD, LOGDEBUG,
	    "\t child (%s) locus %4d -> %4d|%-4d \n",
	    pChild->sID, locusList->pLocusIndex[l],
	    pGenotype->allele[DAD], pGenotype->allele[MOM]);
      genoPos[l] = pGenotype->position;
      /* figure out the index to multilocus genotype array */
      multiLocusIndex += genoPos[l] * pChild->multiLocusAdjust[l];
      for (parent = DAD; parent <= MOM; parent++)
	{
	  chromosome[parent][l] =
	    pParentalPair->ppChildInheritance[parent][child][0];
	  xmissionIndex[parent] +=
	    (chromosome[parent][l] - 1) * powOf3[numLocus - 1 - l];
	}
    }
  ...
  while (TRUE)
	...
	{			/* initialFlag = false */
	  ...
	  /* search for one */
	  for (l = numLocus - 1; l >= 0; l--)
	    {
	      pParentalPair =
		&pHaplo->ppParentalPair[l][pHaplo->pParentalPairInd[l]];
	      if (genoIndex[l] + 1 == pParentalPair->pChildGenoLen[child])
		{		/* reset geno index to 0 */
		  /* adjust multiLocusIndex */
		  pGenotype = pParentalPair->pppChildGenoList[child][0];
		  multiLocusIndex +=
		    (pGenotype->position -
		     genoPos[l]) * pChild->multiLocusAdjust[l];
		  genoIndex[l] = 0;
		  for (parent = DAD; parent <= MOM; parent++)
		    {
			...
			pParentalPair->ppChildInheritance[parent][child][0];
		      xmissionIndex[parent] +=
			(newChromosome[parent] -
			 chromosome[parent][l]) * powOf3[numLocus - 1 - l];
		      chromosome[parent][l] = newChromosome[parent];
		    }
		  genoPos[l] = pGenotype->position;
		  pHaplo->pChildGenoInd[l] = genoIndex[l];
		  if (pParentalPair->pChildGenoLen[child] > 1)
		    KLOG (LOGLIKELIHOOD, LOGDEBUG,
			  "\t child (%s) locus %4d -> %4d|%-4d \n",
			  pChild->sID, locusList->pLocusIndex[l],
			  pGenotype->allele[DAD], pGenotype->allele[MOM]);
		...
		{
		  pGenotype =
		    pParentalPair->pppChildGenoList[child][genoIndex[l] + 1];
		  multiLocusIndex +=
		    (pGenotype->position -
		     genoPos[l]) * pChild->multiLocusAdjust[l];
		  genoIndex[l]++;
		  ...
		  for (parent = DAD; parent <= MOM; parent++)
		    {
		      newChromosome[parent] =
			pParentalPair->
			ppChildInheritance[parent][child][genoIndex[l]];
		      xmissionIndex[parent] +=
			(newChromosome[parent] -
			 chromosome[parent][l]) * powOf3[numLocus - 1 - l];
		      chromosome[parent][l] = newChromosome[parent];
      ...
      if (modelOptions.polynomial == TRUE)
	{
	  newProbPolynomial =
	    timesExp (2,
		      xmissionMatrix[xmissionIndex[DAD]].slot.probPoly[DAD +
								       1], 1,
		      xmissionMatrix[xmissionIndex[MOM]].slot.probPoly[MOM +
								       1], 1,
		      0);
#if 0
	  KLOG (LOGLIKELIHOOD, LOGDEBUG,
		...
		evaluateValue (xmissionMatrix[xmissionIndex[DAD]].
			       slot.probPoly[DAD + 1]),
		evaluateValue (xmissionMatrix[xmissionIndex[MOM]].
			       slot.probPoly[MOM + 1]));
#endif
	}
      else
	{
	  newProb =
	    xmissionMatrix[xmissionIndex[DAD]].slot.prob[DAD + 1] *
	    xmissionMatrix[xmissionIndex[MOM]].slot.prob[MOM + 1];
	  KLOG (LOGLIKELIHOOD, LOGDEBUG,
		"\t xmission prob: %f = %f * %f\n", newProb,
		xmissionMatrix[xmissionIndex[DAD]].slot.prob[DAD +
							     1],
		xmissionMatrix[xmissionIndex[MOM]].slot.prob[MOM + 1]);
      ...
      if (modelOptions.polynomial == TRUE)
	{
	  if (pChild != pProband)
	      ...
	      if (pChild->touchedFlag == 1)
		{
		   ...
		   */
		  *(Polynomial **) childSum = plusExp (2, 1.0, *(Polynomial **) childSum, 1.0, timesExp (2, newProbPolynomial, 1, pChild->pLikelihood[multiLocusIndex].lkslot.likelihoodPolynomial, 1, 0),	//end of timesExp
						       1);
		  //end of plusExp
#if 0
		  KLOG (LOGLIKELIHOOD, LOGDEBUG,
		  ...
		  pTraitParentalPair =
		    &pHaplo->ppParentalPair[locusList->
					    traitLocusIndex][pHaplo->
							     ...
							     [locusList->
							      traitLocusIndex]];
		  *(Polynomial **) childSum = plusExp (2, 1.0, *(Polynomial **) childSum, 1.0, timesExp (2, newProbPolynomial, 1, pTraitParentalPair->pppChildGenoList[child][traitGenoIndex]->penslot.penetrancePolynomial, 1, 0),	//end of timesExp
						       1);
		  //end of plusExp
		}
	      else
	  ...
	  if (pChild != pProband)
		 ...
		 */
		{
		  ...
		  *(double *) childSum +=
		    newProb *
		    pTraitParentalPair->
		    pppChildGenoList[child][traitGenoIndex]->penslot.
		    penetrance;
		  KLOG (LOGLIKELIHOOD, LOGDEBUG,
			"child penetrance %e\n",
			pTraitParentalPair->
		  ...
		  if (prevPattern == pattern)
		      ...
		      for (i = 0; i < 3; i++)
			{
			  ...
			  if (modelOptions.polynomial == TRUE)
			    {
			      if (i > 0
				  && modelOptions.mapFlag == SEX_AVERAGED)
				{
