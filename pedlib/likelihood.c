...
int populate_xmission_matrix (XMission * pMatrix, int totalLoci,
			      void *prob, int cellIndex,
			      int lastHetLoc, int lastHetPattern,
			      int numHet, int loc);
...
//                                   locus,newPenetrance, *((double *)pSum),newProb);
	      ...
	      if (modelOptions.polynomial == TRUE)
		{
		  newProbPolynomial =
		    timesExp (2,
			      xmissionMatrix[newXmissionIndex[DAD]].probPoly,
			      1,
			      xmissionMatrix[newXmissionIndex[MOM]].probPoly,
			      1);
...
//                               evaluateValue(newProbPolynomial),evaluateValue(newPenetrancePolynomial));
		...
		{
		  newProb = xmissionMatrix[newXmissionIndex[DAD]].prob *
		    xmissionMatrix[newXmissionIndex[MOM]].prob;
 ...
 * loc - current locus 
 * pVector - current inheritance pattern
 */
			  ...
			  int numHet, int loc)
{
  int pattern;
#ifndef NO_POLYNOMIAL
  Polynomial *newProbPoly = (Polynomial *) prob;
#endif
  double newProb = *((double *) prob);
  int newCellIndex;
  int newNumHet;
  int newLastHetLoc;
  int newLastHetPattern;
  ...
  for (pattern = 1; pattern <= 3; pattern++)
	...
	newProb = *((double *) prob);
      ...
      newLastHetPattern = lastHetPattern;
      if (pattern != 3)
	{
	  ...
	  if (lastHetLoc != -1)
	    {
	      if (lastHetPattern == pattern)
		{
		  /* no recombination at lastHetPattern & no recombination at current locus  OR
		     recombination at lastHetPattern & recombination at current locus */
		  if (lastHetLoc == loc - 1)
		    {
		      /* two adjacent loci */
			...
			{
			  char vName[100];
			  sprintf (vName, "theta%d", loc - 1);
			  newProbPoly = timesExp (2, (Polynomial *) prob,
						  1,
						  plusExp (2, 1.0,
							   constantExp (1.0),
							   -1.0,
			...
			*((double *) prob) * (1 -
		    ...
		    {
		      /* not two adjacent loci, with homozygous loci in between */
#ifndef NO_POLYNOMIAL
		      if (modelOptions.polynomial == TRUE)
			{
			  char vName1[100], vName2[100];
			  sprintf (vName1, "theta%d", lastHetLoc);
			  sprintf (vName2, "theta%d", loc - 1);
			  newProbPoly = timesExp (2, (Polynomial *) prob,
						  1,
						  plusExp (2, 1.0,
							   timesExp (2,
			...
			newProb =
			  *((double *) prob) *
			  ((1 -
			    locusList->pNextLocusDistance[lastHetLoc]) * (1 -
									  locusList->
									  pPrevLocusDistance
									  [loc])
			   +
			   locusList->pNextLocusDistance[lastHetLoc] *
			   locusList->pPrevLocusDistance[loc]);
#else
		      newProb =
			*((double *) prob) *
			((1 -
	  ...
	  /* update last het info */
	  newLastHetLoc = loc;
	  newLastHetPattern = pattern;
	}			/* end of heterozygous locus processing */
      if (loc == totalLoci - 1)
	{
	  ...
	  if (newNumHet > 0)
	    {
	      ...
	      if (modelOptions.polynomial == TRUE)
		{
		  newProbPoly = timesExp (2, newProbPoly, 1,
					  constantExp (0.5), 1);
		}
	      else
		newProb *= 0.5;
	  ...
	  if (modelOptions.polynomial == TRUE)
	    {
	      pMatrix[newCellIndex].probPoly = newProbPoly;
	    }
	  else
	    pMatrix[newCellIndex].prob = newProb;
#else
	  pMatrix[newCellIndex].prob = newProb;
#endif
	  pMatrix[newCellIndex].numHet = newNumHet;
      ...
      else
	{
	  ...
	  if (modelOptions.polynomial == TRUE)
	    ...
	    populate_xmission_matrix (pMatrix, totalLoci,
				      &newProb, newCellIndex,
				      newLastHetLoc,
				      newLastHetPattern, newNumHet, loc + 1);
#else
	  populate_xmission_matrix (pMatrix, totalLoci,
				    &newProb, newCellIndex,
				    newLastHetLoc,
