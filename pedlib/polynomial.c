...
//      fprintf(stderr,"i=%d, name=%s\n",i,variableList[i]->e.v->vName);
      if (vType == 'D')
	{
	  if (variableList[i]->e.v->vType == vType
	      && variableList[i]->e.v->vAddrD == vD)
	    return variableList[i];
	}
      else if (vType == 'I')
	{
	  if (variableList[i]->e.v->vType == vType
	      && variableList[i]->e.v->vAddrI == vI)
	    return variableList[i];
	}
      else
	{
			  ...
			  if (p[k] == sumList[sIndex]->e.s->sum[k])
			    {
			      ...
			      tempD2 =
				frexp (sumList[sIndex]->e.s->factor[k],
				       &tempI2);
			      if (tempI2 != tempI
				  || (int) (tempD2 * 100000000) !=
				  (int) (tempD * 100000000))
				break;
			    }
			  else
...
//   fprintf(stderr,"new key=%d\n",key);
  ...
  if (functionCallHash[hIndex].num > 0)
	  ...
	  && key == functionCallHash[hIndex].key[binaryStart])
	{
	  ...
	  binaryStart = binaryStart - 1;
	  while (binaryStart >= 0 &&
		 functionCallHash[hIndex].key[binaryStart] ==
		 functionCallHash[hIndex].key[binaryMiddle])
	    binaryStart--;
	  while (binaryEnd < functionCallHash[hIndex].num &&
		 functionCallHash[hIndex].key[binaryEnd] ==
		 functionCallHash[hIndex].key[binaryMiddle])
	    binaryEnd++;
	  if (binaryStart < 0)
	    binaryStart++;
...
//         fprintf(stderr,"\n");
  ...
  functionCallHash[hIndex].key = realloc (functionCallHash[hIndex].key,
					  sizeof (int) *
					  functionCallHash[hIndex].num);
  functionCallHash[hIndex].index =
    realloc (functionCallHash[hIndex].index,
	     sizeof (int) * functionCallHash[hIndex].num);
  if (functionCallHash[hIndex].key == NULL
      || functionCallHash[hIndex].index == NULL)
    {
      fprintf (stderr, "Memory allocation for hash table failed!\n");
      exit (1);
    }
  if (binaryEnd <= functionCallHash[hIndex].num - 2)
	...
	functionCallHash[hIndex].index[i + 1] =
	  functionCallHash[hIndex].index[i];
      }
...
*/
	...
	case T_FUNCTIONCALL:
	  if (strcmp (p->e.f->name, "log10") == 0)
	    ...
	    {
	      p->value =
		gsl_ran_tdist_pdf (p->e.f->para[0]->value,
				   p->e.f->para[1]->value);
	    }
	  else if (strcmp (p->e.f->name, "gsl_cdf_tdist_Q") == 0)
	    {
	      p->value =
		gsl_cdf_tdist_Q (p->e.f->para[0]->value,
				 p->e.f->para[1]->value);
	    }
	  else if (strcmp (p->e.f->name, "gsl_cdf_tdist_P") == 0)
	    {
	      p->value =
		gsl_cdf_tdist_P (p->e.f->para[0]->value,
				 p->e.f->para[1]->value);
	    }
  ...
  for (i = 0; i < functionCallCount; i++)
    {
      fprintf (stderr, "Function Call  id=%ld  name: %s  value=%10.8f\n",
	       functionCallList[i]->id, functionCallList[i]->e.f->name,
	       functionCallList[i]->value);
...
printAllPolynomials ()
{
  ...
  for (i = 0; i < CONSTANT_HASH_SIZE; i++)
	...
	{
	  fprintf (stderr, "Key=%d index=%d constant: ",
  ...
  for (i = 0; i < SUM_HASH_SIZE; i++)
	...
	{
	  fprintf (stderr, "Key=%d index=%d sum: ", sumHash[i].key[j],
		   sumHash[i].index[j]);
  ...
  for (i = 0; i < PRODUCT_HASH_SIZE; i++)
	...
	{
	  fprintf (stderr, "Key=%d index=%d product: ", productHash[i].key[j],
		   productHash[i].index[j]);
  ...
  for (i = 0; i < FUNCTIONCALL_HASH_SIZE; i++)
	...
	{
	  fprintf (stderr, "Key=%d index=%d functionCall: ",
		   functionCallHash[i].key[j], functionCallHash[i].index[j]);
