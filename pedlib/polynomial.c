...
//Count the numbers of polynomials in different categories
void
...
int
isMinusOneExp (struct polynomial *p)
{
  if (p->eType != T_CONSTANT)
    return 0;
  if (p->value == -1.0)
...
//                     binaryStart,binaryMiddle,binaryEnd,constantHash[hIndex].key[binaryStart]);
	  ...
	  && key == constantHash[hIndex].key[binaryStart])
	{
	  ...
	  while (binaryStart >= 0 &&
		 constantHash[hIndex].key[binaryStart] ==
		 constantHash[hIndex].key[binaryMiddle])
	    binaryStart--;
	  //Search for more keys that are equal to the newly generated key
	  while (binaryEnd < constantHash[hIndex].num &&
		 constantHash[hIndex].key[binaryEnd] ==
		 constantHash[hIndex].key[binaryMiddle])
	    binaryEnd++;
	  if (binaryStart < 0)
	    binaryStart++;
	  if (binaryEnd >= constantHash[hIndex].num
	      || key != constantHash[hIndex].key[binaryEnd])
	    binaryEnd--;
...
//  fprintf(stderr,"binaryEnd=%d\n",binaryEnd);
  ...
  constantHash[hIndex].key = realloc (constantHash[hIndex].key,
				      sizeof (int) *
				      constantHash[hIndex].num);
  constantHash[hIndex].index =
    realloc (constantHash[hIndex].index,
...
struct polynomial *
variableExp (double *v, char name[10])
{
  int i;
...
//  fprintf(stderr,"This is a new variable\n");
  ...
  struct variablePoly *vPoly =
    (struct variablePoly *) malloc (sizeof (struct variablePoly));
...
plusExp (int num, ...)
{
  ...
  //Apply memory for container to hold the sum items  
  containerLength = 50;
  factor = (double *) malloc (containerLength * sizeof (double));
  p =
    (struct polynomial **) malloc (containerLength *
...
//     }
      ...
      else if (p1->eType == T_CONSTANT)
	{
      ...
      //The item is either a variable or a product
      else if (p1->eType != T_SUM)
	{
	  ...
	  if (counter == 0 || j >= counter)
	      ...
	      if (counter >= containerLength - 1)
		{
		  containerLength += 50;
		  factor =
		    (double *) realloc (factor,
					containerLength * sizeof (double));
		  p =
      ...
      if (sumHash[hIndex].num > 0)
	{
	  ...
	  if (binaryStart >= 0 && binaryStart <= sumHash[hIndex].num - 1
	      && key == sumHash[hIndex].key[binaryStart])
	    {
	      ...
	      binaryStart = binaryStart - 1;
	      while (binaryStart >= 0 &&
		     sumHash[hIndex].key[binaryStart] ==
		     sumHash[hIndex].key[binaryMiddle])
		binaryStart--;
	      while (binaryEnd < sumHash[hIndex].num &&
		     sumHash[hIndex].key[binaryEnd] ==
		     sumHash[hIndex].key[binaryMiddle])
		binaryEnd++;
	      if (binaryStart < 0)
		binaryStart++;
	      if (binaryEnd >= sumHash[hIndex].num
		  || key != sumHash[hIndex].key[binaryEnd])
		binaryEnd--;
...
//                     fprintf(stderr,"\n");
		  ...
		  if (counter == sumList[sIndex]->e.s->num)
			...
			if (p[k] != sumList[sIndex]->e.s->sum[k]
			    || factor[k] != sumList[sIndex]->e.s->factor[k])
			  break;
	...
	fprintf (stderr, "Memory allocation error: malloc returned NULL!");
      sP->num = counter;
      sP->sum =
	(struct polynomial **) malloc (counter *
				       sizeof (struct polynomial *));
  ...
  for (i = 0; i < num; i++)
    {
      p1 = va_arg (args, struct polynomial *);
      e1 = va_arg (args, int);
//      fprintf(stderr," TimesExp  exponent=%d item No. %d type=%d\n",
//                      e1,i,p1->eType);
//      expPrinting(p1);
//      fprintf(stderr,"\n");
      ...
      else
	{
	  ...
	  //more than one items
	  if (p1->eType == T_VARIABLE || p1->eType == T_SUM)
	    {
		  ...
		  if (counter >= containerLength - 1)
		    {
		      containerLength += 50;
		      exponent =
			(int *) realloc (exponent,
					 containerLength * sizeof (int));
		      p =
			(struct polynomial **) realloc (p,
							containerLength *
							sizeof (struct
			...
			{
			  fprintf (stderr,
				   "Momery allocation error in timesExp()!\n");
			  exit (1);
			}
		    }
		  p[counter] = p1;
      ...
      if (productHash[hIndex].num > 0)
	{
	  ...
	  if (binaryStart >= 0 && binaryStart <= productHash[hIndex].num - 1
	      && key == productHash[hIndex].key[binaryStart])
	    {
	      ...
	      binaryStart = binaryStart - 1;
	      while (binaryStart >= 0 &&
		     productHash[hIndex].key[binaryStart] ==
		     productHash[hIndex].key[binaryMiddle])
		binaryStart--;
	      while (binaryEnd < productHash[hIndex].num &&
		     productHash[hIndex].key[binaryEnd] ==
		     productHash[hIndex].key[binaryMiddle])
		binaryEnd++;
	      if (binaryStart < 0)
		binaryStart++;
	      if (binaryEnd >= productHash[hIndex].num
		  || key != productHash[hIndex].key[binaryEnd])
		binaryEnd--;
...
//            fprintf(stderr,"counter=%d num=%d\n",counter,productList[pIndex]->e.p->num);
		  if (counter == productList[pIndex]->e.p->num)
			...
			if (p[k] != productList[pIndex]->e.p->product[k]
			    || exponent[k] !=
			    productList[pIndex]->e.p->exponent[k])
			  break;
		      //If same
		      if (k >= counter)
			{
      ...
      if (productList == NULL)
	{
      ...
      productHash[hIndex].key = realloc (productHash[hIndex].key,
					 sizeof (int) *
					 productHash[hIndex].num);
      productHash[hIndex].index =
	realloc (productHash[hIndex].index,
		 sizeof (int) * productHash[hIndex].num);
      if (productHash[hIndex].key == NULL
	  || productHash[hIndex].index == NULL)
	{
	  fprintf (stderr, "Memory allocation for hash table failed!\n");
	  exit (1);
	}
...
//      fprintf(stderr,"Key=%d i=%d p[i]->id=%d   FUNCTION_HASH_SIZE=%d\n",
//                          key,i,p[i]->id, FUNCTION_HASH_SIZE);
      key = (key + p[i]->id) % FUNCTION_HASH_SIZE;
//      expPrinting(p[i]);
//      fprintf(stderr,"\n");
    }
  va_end (args);
...
//   fprintf(stderr,"new key=%d\n",key);
  ...
  hIndex = key % FUNCTION_HASH_SIZE;

  //compare the key of this polynomial with the keys of polynomials in the
  //function list
  if (functionHash[hIndex].num > 0)
    {
      binaryStart = 0;
      binaryEnd = functionHash[hIndex].num - 1;
      while (binaryStart <= binaryEnd)
	{
	  binaryMiddle = floor ((binaryStart + binaryEnd) / 2.0);
	  if (key == functionHash[hIndex].key[binaryMiddle])
	    {
	      binaryStart = binaryMiddle;
	      binaryEnd = binaryMiddle;
	      break;
	    }
	  else if (key > functionHash[hIndex].key[binaryMiddle])
	    binaryStart = binaryMiddle + 1;
	  else
	    binaryEnd = binaryMiddle - 1;
	}			//end of while

      if (binaryStart >= 0 && binaryStart <= functionHash[hIndex].num - 1
	  && key == functionHash[hIndex].key[binaryStart])
	{
	  binaryMiddle = binaryStart;
	  binaryEnd = binaryStart + 1;
	  binaryStart = binaryStart - 1;
	  while (binaryStart >= 0 &&
		 functionHash[hIndex].key[binaryStart] ==
		 functionHash[hIndex].key[binaryMiddle])
	    binaryStart--;
	  while (binaryEnd < functionHash[hIndex].num &&
		 functionHash[hIndex].key[binaryEnd] ==
		 functionHash[hIndex].key[binaryMiddle])
	    binaryEnd++;
	  if (binaryStart < 0)
	    binaryStart++;
	  if (binaryEnd >= functionHash[hIndex].num)
	    binaryEnd--;
	  ...
	  for (i = binaryStart; i <= binaryEnd; i++)
	    {
	      fIndex = functionHash[hIndex].index[i];
...
//         fprintf(stderr,"Before insert into functionHash\n");
//         for(i=0;i<functionHash[hIndex].num;i++)
//           fprintf(stderr,"hIndex=%d i=%d this key=%d key=%d index=%d\n",
//                           hIndex,i,key,functionHash[hIndex].key[i],
//                           functionHash[hIndex].index[i]);
//         fprintf(stderr,"\n");


  //Insert the newly built polynomial into the Hash table
  functionHash[hIndex].num++;
  functionHash[hIndex].key = realloc (functionHash[hIndex].key,
				      sizeof (int) *
				      functionHash[hIndex].num);
  functionHash[hIndex].index =
    realloc (functionHash[hIndex].index,
	     sizeof (int) * functionHash[hIndex].num);
  if (functionHash[hIndex].key == NULL || functionHash[hIndex].index == NULL)
    {
      fprintf (stderr, "Memory allocation for hash table failed!\n");
      exit (1);
    }
  if (binaryEnd <= functionHash[hIndex].num - 2)
    for (i = functionHash[hIndex].num - 2; i >= binaryEnd; i--)
      {
	functionHash[hIndex].key[i + 1] = functionHash[hIndex].key[i];
	functionHash[hIndex].index[i + 1] = functionHash[hIndex].index[i];
      }

  functionHash[hIndex].key[binaryEnd] = key;
  functionHash[hIndex].index[binaryEnd] = functionCallCount - 1;


/*
         fprintf(stderr,"After insert into functionHash binaryEnd=%d functionCallCount-1=%d\n",
                  binaryEnd,functionCallCount-1); 
         for(i=0;i<functionHash[hIndex].num;i++)
           fprintf(stderr,"hIndex=%d i=%d this key=%d key=%d index=%d name=%s\n",
                           hIndex,i,key,functionHash[hIndex].key[i],
                           functionHash[hIndex].index[i],functionCallList[functionCallCount-1]->e.f->name);
         fprintf(stderr,"\n");
*/
  free (p);
  ...
  if (l->listNext >= l->listSize)
    {
      l->pList =
	realloc (l->pList,
		 sizeof (struct polynomial *) * (l->listSize + 100));
  ...
  for (j = l->listNext - 1; j >= 0; j--)
	...
	case T_SUM:
	  for (i = 0; i < p->e.s->num; i++)
	    {
	      fprintf (stderr,
		       "list No. %d Type:SUM  item %d factor %f item value%f  sum value %f \n",
		       j, i, p->e.s->factor[i], p->e.s->sum[i]->value,
		       p->value);
	    }
	  break;
	...
	case T_PRODUCT:
	  for (i = 0; i < p->e.p->num; i++)
	    {
	      fprintf (stderr,
		       "list No. %d Type:Product  item %d item value %f exponent %d product value %f \n",
		       j, i, p->e.p->product[i]->value, p->e.p->exponent[i],
		       p->value);
	    }
	  break;
	...
	case T_FUNCTIONCALL:
	  fprintf (stderr,
		   "list No. %d type:CONSTANT CALL  name=%s  value=%f\n", j,
		   p->e.f->name, p->value);
	  break;
...
polynomialInitialization ()
{
  ...
  productHash =
    (struct hashStruct *) malloc (PRODUCT_HASH_SIZE *
				  sizeof (struct hashStruct));
  functionHash =
    (struct hashStruct *) malloc (FUNCTION_HASH_SIZE *
				  sizeof (struct hashStruct));
  ...
  for (i = 0; i < FUNCTION_HASH_SIZE; i++)
    {
      functionHash[i].num = 0;
      functionHash[i].index = NULL;
      functionHash[i].key = NULL;
    }
...
clear ()
{
  ...
  for (j = 0; j < FUNCTION_HASH_SIZE; j++)
    {
      if (functionHash[j].num > 0)
	{
	  free (functionHash[j].index);
	  free (functionHash[j].key);
	}
    }
  ...
  for (i = 0; i < sumCount; i++)
      ...
      for (j = 0; j < sumList[i]->e.s->num; j++)
	{
	  fprintf (stderr, "   %d  %ld  %10.8f  %10.8f\n",
		   j, sumList[i]->e.s->sum[j]->id,
		   sumList[i]->e.s->sum[j]->value,
		   sumList[i]->e.s->factor[j]);
	}
    }
