...
timesExp (int num, ...)
{
  ...
  if (counter_s2 > 0) {
    memcpy (&exponentProd[counter_v2],
	    &exponent_s2[0], sizeof (int) * counter_s2);
    memcpy (&pProd[counter_v2], &p_s2[0],
	    sizeof (struct polynomial *) * counter_s2);
  }
  if (counter_f2 > 0) {
    memcpy (&exponentProd[counter_v2 + counter_s2],
	    &exponent_f2[0], sizeof (int) * counter_f2);
    memcpy (&pProd[counter_v2 + counter_s2],
	    &p_f2[0], sizeof (struct polynomial *) * counter_f2);
  }
  ...
  //The result polynomial is a product polynomial
    ...
    rp = (struct polynomial *)
      malloc (sizeof (struct polynomial));
    if (rp == NULL)
      fprintf (stderr, "Memory allocation error: malloc returned NULL!");
    rp->eType = T_PRODUCT;
    pP = (struct productPoly *)
      malloc (sizeof (struct productPoly));
    if (pP == NULL)
      fprintf (stderr, "Memory allocation error: malloc returned NULL!");
    pP->num = counterProd;
    pP->product =
...
functionCallExp (int num, ...)
{
  ...
  if (functionCallHash[hIndex].num > 0) {
	...
	if (strcmp
	    (fName,
	     functionCallList[fIndex]->e.f->name) == 0
  ...
  fP = (struct functionPoly *)
    malloc (sizeof (struct functionPoly));
  if (rp == NULL || fP == NULL)
    fprintf (stderr, "Memory allocation error: malloc returned NULL!");
  fP->paraNum = num - 1;
  fP->para =
  ...
  insertHashTable (&functionCallHash[hIndex],
		   location, key, functionCallCount - 1);
  free (p);
  return rp;
};
...
buildPolyList ()
{
  ...
  l->listSize = 100;
  l->listNext = 0;
  l->pList = (struct polynomial **)
    malloc (sizeof (struct polynomial *) * l->listSize);
  return l;
};
...
//             p->value*=pow(p->e.p->product[i]->value,p->e.p->exponent[i]);
	...
	p->value = gsl_ran_ugaussian_pdf (p->e.f->para[0]->value);
      } else if (strcmp (p->e.f->name, "gsl_cdf_ugaussian_Q")
		 == 0) {
	p->value = gsl_cdf_ugaussian_Q (p->e.f->para[0]->value);
      } else if (strcmp (p->e.f->name, "gsl_cdf_ugaussian_P")
		 == 0) {
	...
	fprintf (stderr,
		 "unknown function name %s in polynomials\n", p->e.f->name);
	exit (1);
      }
      break;
    default:
...
printPolyList (struct polyList *l)
{
  ...
  for (j = l->listNext - 1; j >= 0; j--) {
    ...
    case T_VARIABLE:
      fprintf (stderr,
	       "  type:VAR  name=%s  value=%f type=%c",
	...
	fprintf (stderr,
		 ...
		 i, p->e.s->sum[i]->eType,
		 p->e.s->sum[i]->id, p->e.s->factor[i],
		 p->e.s->sum[i]->value, p->value);
      }
      break;
    case T_PRODUCT:
...
polynomialInitialization ()
{
  ...
  functionCallListLength = 10000;
  functionCallCount = 0;
  functionCallList = (struct polynomial **)
    malloc (functionCallListLength * sizeof (struct polynomial *));
...
{
  int i, j;
  int constantHashCount = 0, variableHashCount =
    0, sumHashCount = 0, productHashCount = 0, functionHashCount = 0;
  int constantHashSize = 0, variableHashSize =
    0, sumHashSize = 0, productHashSize = 0, functionHashSize = 0;
  ...
  for (i = 0; i < FUNCTIONCALL_HASH_SIZE; i++) {
	...
	fprintf (stderr, "(%d %d %d) ", j,
		 functionCallHash[i].index[j], functionCallHash[i].key[j]);
      }
      fprintf (stderr, "\n");
    }
  }
  ...
  fprintf (stderr,
	   ...
	   constantHashCount, variableHashCount,
	   sumHashCount, productHashCount, functionHashCount);
  fprintf (stderr,
	   "constantHashSize: %d   variableHashSize: %d   sumHashSize: %d    productHashSize: %d    functionHashSize: %d\n",
	   constantHashSize, variableHashSize,
	   sumHashSize, productHashSize, functionHashSize);
}
