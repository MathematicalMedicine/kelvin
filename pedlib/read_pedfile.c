...
read_person (char *sPedfileName, int lineNo, char *pLine, Person * pPerson)
{
      ...
      while (j < numTrait)
	{
	  ...
	  if (pTrait->type == DICHOTOMOUS)
		  ...
		  pTrait->unknownTraitValue + 0.000001)
		{
		  pPerson->ppTraitKnown[i][j] = 1;
		  if ( (pPerson->ppOrigTraitValue[i][j] <=
			modelOptions.affectionStatus[AFFECTION_STATUS_UNAFFECTED] - 0.000001
			|| pPerson->ppOrigTraitValue[i][j] >=
			modelOptions.affectionStatus[AFFECTION_STATUS_UNAFFECTED] + 0.000001)
		       && (pPerson->ppOrigTraitValue[i][j] <=
			   modelOptions.affectionStatus[AFFECTION_STATUS_AFFECTED] - 0.000001
			   || pPerson->ppOrigTraitValue[i][j] >=
			   modelOptions.affectionStatus[AFFECTION_STATUS_AFFECTED] + 0.000001)
		       )
  ...
  while (numMarker < originalLocusList.numLocus)
      ...
      else
	{
	  ...
	  KASSERT ( ret >= 0, "Line %d in pedfile %s contains a genotype with unkown allele %s at locus %s.\n",
...
read_ccfile(char *ccFileName, PedigreeSet *pPedigreeSet)
{
  ...
  KASSERT (fpFile != NULL,
	   "Can't open case control count file %s for read. Exiting!", ccFileName);
  while(fgets(line, MAX_LINE_LEN, fpFile) != NULL)
    {
      lineNo++;
      if(is_line_blank_or_comment(line))
	continue;
      numRet = sscanf(line, "%s %n", sCurrPedLabel, &pos);
      KASSERT(numRet == 1, 
	      "Can't get pedigree label from line no %d in case ctrl count file %s.\n",
	      lineNo, ccFileName);
      pLine = &line[pos];
      pPed = NULL;
      ...
      while( i < originalLocusList.numLocus)
	{
	  numRet = sscanf (pLine, "%d %n", &pPed->pCount[i], &pos);
	  KASSERT(numRet == 1, 
		  "Can't get count information for pedigree %s at locus %d.\n", 
		  sCurrPedLabel, i+1);
	  pLine = &pLine[pos];
	  i++;
	}
