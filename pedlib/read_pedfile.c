...
create_person (Pedigree * pPed, char *sID)
{
  ...
  pPerson->pTransmittedAlleles[MOM] = (unsigned int *) malloc (sizeof (unsigned int) * size);
  pPerson->pTransmittedAlleles[DAD] = (unsigned int *) malloc (sizeof (unsigned int) * size);
  pPerson->pNonTransmittedAlleles[MOM] = (unsigned int *) malloc (sizeof (unsigned int) * size);
  pPerson->pNonTransmittedAlleles[DAD] = (unsigned int *) malloc (sizeof (unsigned int) * size);
  ...
  if (pNucFam->numChildren >= pNucFam->maxNumChildren)
	...
	ppNew[i] = pNucFam->ppChildrenList[i];
      ...
      pNucFam->ppChildrenList = (Person **) realloc (pNucFam->ppChildrenList, 
						     sizeof (Person *) * pNucFam->maxNumChildren);
    }
