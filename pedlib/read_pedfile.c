...
read_pedfile (char *sPedfileName, PedigreeSet * pPedigreeSet)
{
  FILE *fpPedfile = NULL;
  int lineNo = 0;
  char line[MAX_LINE_LEN];	/* temporary buffer to read a line from pedfile */
  ...
  while (1)
    {
      ...
      if (lastFlag == 0 && fgets (line, MAX_LINE_LEN, fpPedfile) == NULL)
	lastFlag = 1;
      if (lastFlag == 0)
	{
	  lineNo++;
	  ...
	  numRet =
	    sscanf (line, "%s %s %n", sCurrPedLabel, sCurrPersonLabel, &pos);
	  KASSERT (numRet == 2,
  ...
  while (i < originalLocusList.numTraitLocus)
      ...
      while (j < numTrait)
	{
	  ...
	  if (pTrait->type == DICHOTOMOUS)
		...
		{
		  /* assuming affection status 0 as unknown */
...
create_person (Pedigree * pPed, char *sID)
{
  ...
  size = originalLocusList.numLocus;
  pPerson->ppTransmittedAlleles[MOM] = (int **) malloc (sizeof (int) * size);
  pPerson->ppTransmittedAlleles[DAD] = (int **) malloc (sizeof (int) * size);
  pPerson->ppNonTransmittedAlleles[MOM] =
    (int **) malloc (sizeof (int) * size);
  pPerson->ppNonTransmittedAlleles[DAD] =
    (int **) malloc (sizeof (int) * size);
  size = originalLocusList.alleleSetLen;
  for (i = 0; i < originalLocusList.numLocus; i++)
    {
      pPerson->ppTransmittedAlleles[MOM][i] =
	(int *) malloc (sizeof (int) * size);
      pPerson->ppTransmittedAlleles[DAD][i] =
	(int *) malloc (sizeof (int) * size);
      pPerson->ppNonTransmittedAlleles[MOM][i] =
	(int *) malloc (sizeof (int) * size);
      pPerson->ppNonTransmittedAlleles[DAD][i] =
	(int *) malloc (sizeof (int) * size);
    }
...
setup_nuclear_families (Pedigree * pPed)
{
  ...
  pDoneList = (int *) malloc (sizeof (int) * pPed->numPerson);
  ...
  if (pNucFam->numChildren >= pNucFam->maxNumChildren)
    {
      ...
      ppNew = (Person **) malloc (sizeof (pChild) * pNucFam->maxNumChildren);
      /* copy old stuff over */
...
create_nuclear_family (Pedigree * pPed)
{
  ...
  if (oldNumNucFam >= pPed->maxNuclearFamily)
	...
	(NuclearFamily **) realloc (pPed->ppNuclearFamilyList,
				    sizeof (pNew) * pPed->maxNuclearFamily);
    }
...
print_nuclear_family (FILE * fp, Pedigree * pPed)
{
  ...
  logMsg (LOGPEDFILE, LOGDEBUG, "Pedigree %s Nuclear Families (%d): \n",
	  pPed->sPedigreeID, pPed->numNuclearFamily);
  for (i = 0; i < pPed->numNuclearFamily; i++)
    {
      pNucFam = pPed->ppNuclearFamilyList[i];
      logMsg (LOGPEDFILE, LOGDEBUG, "  %3d: Dad - %s Mom - %s \n",
	      i, pNucFam->pParents[DAD]->sID, pNucFam->pParents[MOM]->sID);
      logMsg (LOGPEDFILE, LOGDEBUG, "       Children - ");
      for (j = 0; j < pNucFam->numChildren; j++)
	logMsg (LOGPEDFILE, LOGDEBUG, "%s  ",
		pNucFam->ppChildrenList[j]->sID);
      logMsg (LOGPEDFILE, LOGDEBUG, "\n");

      /* now print out connector information */
      pNucFamConnector = pNucFam->pUpConnectors;
      logMsg (LOGPEDFILE, LOGDEBUG, "       Up Connectors:\n");
  ...
  logMsg (LOGPEDFILE, LOGDEBUG, "    Founder Nuclear Families:\n");
  logMsg (LOGPEDFILE, LOGDEBUG, "      ");
  for (i = 0; i < pPed->numFounderNuclearFamily; i++)
    logMsg (LOGPEDFILE, LOGDEBUG, "%d ",
  ...
  if (pPed->numFounderNuclearFamily >= pPed->maxNumFounderNuclearFamily)
    {
      ...
      ppNew = (NuclearFamily **) malloc (sizeof (pNucFam) *
