	  ...
	  if (pTrait->functionQT == QT_FUNCTION_NORMAL)
		...
		{ /* point PDF */
		  ...
		  if (modelOptions.polynomial == TRUE)
			...
			if(pTrait->maxFlag && trait >= pTrait->max - 0.000001 && 
			  ...
			  {
			    *(Polynomial **) pen =
			      functionCallExp (2, "gsl_ran_ugaussian_pdf",
					       tempPoly);
			  }
	              ...
	              tempPoly = timesExp(2, tempPoly, 1,
					  timesExp(2, 
						  variableExp (&pTrait->
							       ...
							       NULL, 'D', "stddev"),
						   1,
						   functionCallExp(2,"sqrt", timesExp(2,
										      plusExp(2, 
											      1.0,
		   ...
		   trait <= pTrait->lessCutoffFlag + 0.000001))
		{
		  ...
		  if (modelOptions.polynomial == TRUE)
		  ...
		  else
		    {
		      ...
		      *(double *) pen = gsl_cdf_chisq_P(pTrait->cutoffValue[liabilityClass - 1],
						       mean);
		    }
#else
		  *(double *) pen = gsl_cdf_chisq_P(pTrait->cutoffValue[liabilityClass - 1],
			...
			trait <= pTrait->moreCutoffFlag + 0.000001))
		{
		  ...
		  if (modelOptions.polynomial == TRUE)
		    ...
		    {
		      *(double *) pen = gsl_cdf_chisq_Q(pTrait->cutoffValue[liabilityClass - 1],
						       mean);
		    }
#else
		  *(double *) pen = gsl_cdf_chisq_Q(pTrait->cutoffValue[liabilityClass - 1],
...
setup_LD_haplotype_freq (LDLoci * pLDLoci)
{
  ...
  for (i = 0; i < numAllele1 - 1; i++)
      ...
      for (j = 0; j < numAllele2 - 1; j++)
	{
      ...
      KASSERT(pLDLoci->ppHaploFreq[i][j]>0, 
	      "Haplotype frequency is NEGATIVE - check your marker frequency file please.\n");
  ...
  if (numMarkers == total)
    {
      ...
      for (i = 0; i < originalLocusList.numLocus; i++)
	{
	  if (originalLocusList.ppLocusList[i]->locusType ==
	      LOCUS_TYPE_MARKER)
	    {
	      ret =
		    ...
		    int mapFlag)
{
  ...
  if ((originalLocusList.ppLocusList[first]->locusType == LOCUS_TYPE_MARKER &&
       ...
       locus < first) ||
      thisPos[MAP_SEX_AVERAGE] < firstPos[MAP_SEX_AVERAGE] || (directionFlag == DIRECTION_LEFT &&
			     (thisPos[MAP_SEX_AVERAGE] > firstPos[MAP_SEX_AVERAGE] - ERROR_MARGIN) &&
			     (thisPos[MAP_SEX_AVERAGE] < firstPos[MAP_SEX_AVERAGE] + ERROR_MARGIN)))
      ...
      for(j=0; j < 3; j++)
	{
      ...
      pLocusList->pNextLocusDistance[0][0] = pLocusList->pPrevLocusDistance[0][1] =
	cm_to_recombination_fraction (firstPos[0] - thisPos[0], map.mapFunction);
      if(modelOptions.mapFlag == SEX_AVERAGED)
	{
	  pLocusList->pNextLocusDistance[1][0] = pLocusList->pPrevLocusDistance[1][1] =
	    pLocusList->pNextLocusDistance[0][0];
	  pLocusList->pNextLocusDistance[2][0] = pLocusList->pPrevLocusDistance[2][1] =
	    pLocusList->pNextLocusDistance[0][0];
	}
      else
	{
	  for(j=1; j <=2; j++)
	    {
	      pLocusList->pNextLocusDistance[j][0] = pLocusList->pPrevLocusDistance[j][1] =
		cm_to_recombination_fraction (firstPos[j] - thisPos[j], map.mapFunction);
	    }
	}
      /* done */
      return 0;
  ...
  if ((originalLocusList.ppLocusList[first]->locusType == LOCUS_TYPE_MARKER &&
       originalLocusList.ppLocusList[locus]->locusType == LOCUS_TYPE_MARKER &&
       locus > last) ||
      thisPos[MAP_SEX_AVERAGE] > lastPos[MAP_SEX_AVERAGE] || (directionFlag == DIRECTION_RIGHT &&
			    (thisPos[MAP_SEX_AVERAGE] > lastPos[MAP_SEX_AVERAGE] - ERROR_MARGIN) &&
			    (thisPos[MAP_SEX_AVERAGE] < lastPos[MAP_SEX_AVERAGE] + ERROR_MARGIN)))
      ...
      else
	{
	  for(j=1; j<=2; j++)
		...
		pLocusList->pNextLocusDistance[j][numLocus - 1] =
		cm_to_recombination_fraction (thisPos[j] - lastPos[j], map.mapFunction);
	    }
	}
  ...
  while (i < numLocus)
    {
      ...
      currPos = get_map_position (first);
      if ((originalLocusList.ppLocusList[curr]->locusType == LOCUS_TYPE_MARKER
	   && originalLocusList.ppLocusList[locus]->locusType ==
	   LOCUS_TYPE_MARKER && locus < curr) || thisPos[MAP_SEX_AVERAGE] < currPos[MAP_SEX_AVERAGE]
	  || (directionFlag == DIRECTION_LEFT
	      && (thisPos[MAP_SEX_AVERAGE] > currPos[MAP_SEX_AVERAGE] - ERROR_MARGIN)
	      && (thisPos[MAP_SEX_AVERAGE] < currPos[MAP_SEX_AVERAGE] + ERROR_MARGIN)))
	{
	  ...
	  for (j = numLocus - 1; j >= i; j--)
	    {
	      ...
	      for(k=0; k < 3; k++)
		{
		  pLocusList->pPrevLocusDistance[k][j + 1] =
		    pLocusList->pPrevLocusDistance[k][j];
	  ...
	  pLocusList->pPrevLocusDistance[0][i] =
	    ...
	    cm_to_recombination_fraction (thisPos[0] - leftPos[0], map.mapFunction);
	  /* calculate the distance */
	  ...
	  if(modelOptions.mapFlag == SEX_SPECIFIC)
	      ...
	      for(k=1; k <= 2; k++)
		{
		  pLocusList->pPrevLocusDistance[k][i] =
		    ...
		    cm_to_recombination_fraction (thisPos[k] - leftPos[k], map.mapFunction);
		  /* calculate the distance */
		  pLocusList->pNextLocusDistance[k][i] =
		    pLocusList->pPrevLocusDistance[k][i + 1] =
		    cm_to_recombination_fraction (firstPos[k] - thisPos[k], map.mapFunction);
