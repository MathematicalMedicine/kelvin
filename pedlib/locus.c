	  ...
	  if (pTrait->functionQT == QT_FUNCTION_NORMAL)
		   ...
		   trait <= pTrait->lessCutoffFlag + 0.000001))
		{
		  /* the cutoff is number of SD, so it's already been converted to X in N(0,1) */
                  ...
                  if (modelOptions.polynomial == TRUE)
		    {
		      tempPoly=timesExp(2, plusExp(2, 1.0,variableExp(&pTrait->cutoffValue[liabilityClass-1], NULL,'D',"cutoff"),
		      ...
		      *(Polynomial **) pen = functionCallExp(2,"gsl_cdf_ugaussian_P", tempPoly);
			...
			trait <= pTrait->moreCutoffFlag + 0.000001))
		{
		  /* the cutoff is number of SD, so it's already been converted to X in N(0,1) */
                  ...
                  if (modelOptions.polynomial == TRUE)
		    {
		      ...
		      *(Polynomial **) pen = functionCallExp(2,"gsl_cdf_ugaussian_Q", tempPoly);
