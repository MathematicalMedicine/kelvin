  ...
  for (i = 0; i < pPedigree->numPerson; i++)
      ...
      else
	{
	  if (pPerson->pPhenotypeList[0][locus] == 0 &&
		...
		{
		  for (allele2 = allele1; allele2 <= pLocus->numAllele;
		       allele2++)
		    {
	      ...
	      if (modelOptions.polynomial == TRUE)
		{
		  ...
		  if (allele1 != allele2)
			...
			add_genotype (&pPerson->ppGenotypeList[locus],
		      ...
		      pGenotype->pDualGenotype = pGenotype2;
		      pGenotype2->pDualGenotype = pGenotype;
		      pGenotype2->penslot.penetrancePolynomial =
			penPolynomial;
		    }
		}
	      else
	  ...
	  if (pTrait->functionQT == QT_FUNCTION_NORMAL)
		...
		{		/* point PDF */
		  ...
		  if (modelOptions.polynomial == TRUE)
			...
			{
			  *(Polynomial **) pen =
			    functionCallExp (2, "gsl_cdf_ugaussian_P",
					     tempPoly);
			}
		      else
			...
			{
			  *(Polynomial **) pen =
			    functionCallExp (2, "gsl_cdf_ugaussian_Q",
					     tempPoly);
			}
		      else
			{
		   ...
		   trait <= pTrait->lessCutoffFlag + 0.000001))
		{
		  ...
		  if (modelOptions.polynomial == TRUE)
		  ...
		  else
		    {
		      ...
		      *(double *) pen =
			gsl_cdf_chisq_P (pTrait->
					 cutoffValue[liabilityClass - 1],
					 mean);
		    }
#else
		  *(double *) pen =
		    gsl_cdf_chisq_P (pTrait->cutoffValue[liabilityClass - 1],
				     mean);
#endif
		}
	      else if (pTrait->type == COMBINED &&
			...
			trait <= pTrait->moreCutoffFlag + 0.000001))
		{
		  ...
		  if (modelOptions.polynomial == TRUE)
		    ...
		    {
		      *(double *) pen =
			gsl_cdf_chisq_Q (pTrait->
					 cutoffValue[liabilityClass - 1],
					 mean);
		    }
#else
		  *(double *) pen =
		    gsl_cdf_chisq_Q (pTrait->cutoffValue[liabilityClass - 1],
				     mean);
#endif
		}
	      else
		    ...
		    int mapFlag)
{
  ...
  if ((originalLocusList.ppLocusList[first]->locusType == LOCUS_TYPE_MARKER &&
       ...
       locus < first) ||
      thisPos[MAP_SEX_AVERAGE] < firstPos[MAP_SEX_AVERAGE]
  ...
  if ((originalLocusList.ppLocusList[last]->locusType == LOCUS_TYPE_MARKER &&
       originalLocusList.ppLocusList[locus]->locusType == LOCUS_TYPE_MARKER &&
       locus > last) ||
      thisPos[MAP_SEX_AVERAGE] > lastPos[MAP_SEX_AVERAGE]
	...
	cm_to_recombination_fraction (thisPos[0] - lastPos[0],
				      map.mapFunction);
      if (modelOptions.mapFlag == SEX_AVERAGED)
	{
