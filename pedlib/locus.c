	  ...
	  if (pTrait->functionQT == QT_FUNCTION_NORMAL)
		   ...
		   trait <= pTrait->lessCutoffFlag + 0.000001))
		{
		  /* the cutoff is number of SD, so it's already been converted to X in N(0,1) */
                  ...
                  if (modelOptions.polynomial == TRUE)
                  {
                      *(Polynomial **) pen = functionCallExp(2,"gsl_cdf_ugaussian_P",
                                                               variableExp(&pTrait->cutoffValue[liabilityClass-1], NULL,'D',"cutoff"));
                  }
                  else
                  {
		    *(double *)pen = gsl_cdf_ugaussian_P(pTrait->cutoffValue[liabilityClass-1]);
                  }
#else
		    *(double *)pen = gsl_cdf_ugaussian_P(pTrait->cutoffValue[liabilityClass-1]);
#endif
			...
			trait <= pTrait->moreCutoffFlag + 0.000001))
		{
		  /* the cutoff is number of SD, so it's already been converted to X in N(0,1) */
                  ...
                  if (modelOptions.polynomial == TRUE)
                  {
                      *(Polynomial **) pen = functionCallExp(2,"gsl_cdf_ugaussian_Q",
                                                               variableExp(&pTrait->cutoffValue[liabilityClass-1], NULL,'D',"cutoff"));
                  }
                  else
                  {
                 
		    *(double *)pen = gsl_cdf_ugaussian_Q(pTrait->cutoffValue[liabilityClass-1]);
                  }
#else
		  *(double *)pen = gsl_cdf_ugaussian_Q(pTrait->cutoffValue[liabilityClass-1]);
#endif
	      ...
	      else
		{
		    ...
		    *(Polynomial **) pen = functionCallExp(2,"gsl_ran_ugaussian_pdf",tempPoly);
		   ...
		   trait <= pTrait->lessCutoffFlag + 0.000001))
		{
                  ...
                  if (modelOptions.polynomial == TRUE)
                  {
                      *(Polynomial **) pen = functionCallExp(3,"gsl_cdf_tdist_P",
                                                               variableExp(&pTrait->cutoffValue[liabilityClass-1],NULL,'D', "cutoff"),
                                                               variableExp(NULL, &pTrait->dfQT,'I',"df"));
                  }
                  else
		  {
		    *(double *) pen = gsl_cdf_tdist_P(pTrait->cutoffValue[liabilityClass-1], df);
                  }
#else
			...
			trait <= pTrait->moreCutoffFlag + 0.000001))
		{
                  ...
                  if (modelOptions.polynomial == TRUE)
                  {
                      *(Polynomial **) pen = functionCallExp(3,"gsl_cdf_tdist_Q",
                                                               variableExp(&pTrait->cutoffValue[liabilityClass-1],NULL,'D', "cutoff"),
                                                               variableExp(NULL,&pTrait->dfQT,'I',"df"));
		    ...
		    {
		      *(double *) pen = gsl_cdf_tdist_Q(pTrait->cutoffValue[liabilityClass-1], df);
		    }
#else
		  *(double *) pen = gsl_cdf_tdist_Q(pTrait->cutoffValue[liabilityClass-1], df);
#endif
                       ...
                       tempPoly = timesExp(2, plusExp(2,  1.0, variableExp(&pPerson->ppTraitValue[locus][i], NULL,'D',"trait"),
							 -1.0, variableExp(&pTrait->means[liabilityClass - 1][allele1 - 1][allele2 - 1],NULL,'D',"mean")
    					       ...
    					       1,
                     			       variableExp(&pTrait->stddev[liabilityClass - 1][allele1 - 1][allele2 - 1],NULL,'D',"stddev"),
                 		  	       -1);                                                                 
                       *(Polynomial **) pen = functionCallExp(3,"gsl_ran_tdist_pdf",  tempPoly,  variableExp(NULL, &pTrait->dfQT,'I',"df"));
                  }
                  else
                  {
		    /*
