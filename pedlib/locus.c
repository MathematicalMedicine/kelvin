 ...
 * MARKERS SHOULD BE LISTED IN MAP ORDER. 
 * 
...
read_mapfile (char *sMapfileName)
{
  ...
  MapUnit *pPrevMarker; /* previous marker */
  char *p;
  int chr;
  ...
  while (fgets(line, MAX_LINE_LEN, fp)) 
	...
	continue;
      ...
      if(pPrevMarker != NULL && (pPrevMarker->mapPos[MAP_SEX_AVERAGE] > pMarker->mapPos[MAP_SEX_AVERAGE] ||
				 pPrevMarker->mapPos[MAP_MALE] > pMarker->mapPos[MAP_MALE] ||
				 pPrevMarker->mapPos[MAP_FEMALE] > pMarker->mapPos[MAP_FEMALE] ))
	{
  ...
  if(numMarkers == total)
      ...
      for(i = 0; i < originalLocusList.numLocus; i++)
	{
	  if(originalLocusList.ppLocusList[i]->locusType == LOCUS_TYPE_MARKER ) 
	    {
	      ret = add_analysis_locus(pLocusList, i, DIRECTION_LEFT, map.mapFunction);
	    }
	}
      return 0;
  ...
  else
    {
      ...
      *pLeftMarker = pick_left_flanking_marker(*pLeftMarker, end, traitPosition, mapFlag);
      if(*pLeftMarker >= 0) {
	numMarkerSelected++;
	ret = add_analysis_locus(pLocusList, *pLeftMarker, DIRECTION_LEFT, map.mapFunction);
      }
	...
	{
	  numMarkerSelected++;
	  ret = add_analysis_locus(pLocusList, rightMarker, DIRECTION_LEFT, map.mapFunction);
	}
