	  ...
	  if (pTrait->functionQT == QT_FUNCTION_NORMAL)
		   ...
		   trait <= pTrait->lessCutoffFlag + 0.000001))
		{
		  ...
		  if (modelOptions.polynomial == TRUE)
		    ...
		    {
		      temp = pTrait->cutoffValue[liabilityClass - 1]*stddev - mean;
		    ...
		    int mapFlag)
{
  ...
  if ((originalLocusList.ppLocusList[first]->locusType == LOCUS_TYPE_MARKER &&
      ...
      for (i = numLocus - 1; i >= 0; i--)
	{
	  ...
	  pLocusList->pPrevLocusDistance[i + 1] =
	    pLocusList->pPrevLocusDistance[i];
	  pLocusList->pNextLocusDistance[i + 1] =
  ...
  if ((originalLocusList.ppLocusList[first]->locusType == LOCUS_TYPE_MARKER &&
...

    {
      ...
      /* calculate the distance */
      pLocusList->pPrevLocusDistance[numLocus] =
	pLocusList->pNextLocusDistance[numLocus - 1] =
  ...
  while (i < numLocus)
    {
      ...
      if ((originalLocusList.ppLocusList[curr]->locusType == LOCUS_TYPE_MARKER
	   ...
	   LOCUS_TYPE_MARKER && locus < curr) || thisPos < currPos
	      ...
	      && (thisPos < currPos + ERROR_MARGIN)))
	{
	  ...
	  for (j = numLocus - 1; j >= i; j--)
		...
		pLocusList->pPrevLocusDistance[j];
	      pLocusList->pNextLocusDistance[j + 1] =
		pLocusList->pNextLocusDistance[j];
	    }
	  pLocusList->numLocus++;
	  ...
	  pLocusList->pNextLocusDistance[i] =
	    pLocusList->pPrevLocusDistance[i + 1] =
	    cm_to_recombination_fraction (firstPos - thisPos,
					  map.mapFunction);
	  /* done */
	  return 0;
	}
