...
int count_alleles (int *pAlleleSet, int alleleSetLen);
int set_allele_bit (int allele, int *pBits);
...
int print_allele_set (int *pAllele, int len);
int print_locus_allele_set (Locus * pLocus, int alleleSetLen);

int add_allele_set (int locus, int *pAlleleBits);
int find_allele_set (Locus * pLocus, int *pAlleleBits, int alleleSetLen);
int is_subset (int *pAlleleBits1, int *pAlleleBits2, int alleleSetLen);
...
identify_transmitted_alleles (int locus, Pedigree * pPedigree)
{
  ...
  for (i = 0; i < numPerson; i++)
	...
	{
	  pPerson->ppTransmittedAlleles[MOM][locus][k] = 0;
	  pPerson->ppTransmittedAlleles[DAD][locus][k] = 0;
	  pPerson->ppNonTransmittedAlleles[MOM][locus][k] = 0;
	  pPerson->ppNonTransmittedAlleles[DAD][locus][k] = 0;
	}
      pDonePerson[i] = FALSE;
    }
  ...
  for (i = 0; i < numPerson; i++)
      ...
      while (pGenotype)
	{
	  ...
	  for (k = 0; k < alleleSetLen; k++)
	    {
	      pPerson->ppNonTransmittedAlleles[DAD][locus][k] |=
		pGenotype->pAlleleBits[DAD][k];
	      pPerson->ppNonTransmittedAlleles[MOM][locus][k] |=
      ...
      for (j = DAD; j <= MOM; j++)
	{
	  ...
	  for (m = 1; m <= numAllele; m++)
	    {
	      mask = 1;
	      mask = mask << (m % INT_BITS);
	      pPerson->ppNonTransmittedAlleles[j][locus][m / INT_BITS] |=
		mask;
	    }
	}
  ...
  for (i = 0; i < numPerson; i++)
	...
	{
	  if ((alleleCount =
	       count_alleles (pPerson->ppNonTransmittedAlleles[DAD][locus],
			      alleleSetLen)) <= 1)
	    pPerson->recodeFlag = FALSE;
	  if ((alleleCount =
	       count_alleles (pPerson->ppNonTransmittedAlleles[MOM][locus],
			      alleleSetLen)) <= 1)
	    pPerson->recodeFlag = FALSE;
	}
				    ...
				    Person * pChild, int locus)
{
  ...
  if (!(modelOptions.sexLinked == TRUE &&
	pFounder->sex == MALE && pChild->sex == MALE))
    {
      ...
      if (pChild->pTypedFlag[locus] != TRUE)
	{
	  ...
	  while (pChildChild != NULL)
	      ...
	      for (j = DAD; j <= MOM; j++)
		{
		  for (k = 0; k < alleleSetLen; k++)
		    {
		      pFounder->ppNonTransmittedAlleles[j][locus][k] &=
			pChild->ppNonTransmittedAlleles[sex][locus][k];
		      pFounder->ppTransmittedAlleles[j][locus][k] |=
			pChild->ppTransmittedAlleles[sex][locus][k];
		    }
		}
	    }
    ...
    }				/* end of not sex linked, dad and son pair */
...
int
set_allele_bit (int allele, int *pBits)
{
  unsigned int mask;
  ...
  mask = mask << (allele % INT_BITS);
  /* set the right integer in the set */
  pBits[allele / INT_BITS] |= mask;
 ...
 * */
int
count_alleles (int *pAlleleSet, int alleleSetLen)
{
  int i, j;
  int count = 0;
  unsigned int mask;
...
recode_genotype (int locus, Pedigree * pPedigree)
{
  ...
  for (i = 0; i < numPerson; i++)
      ...
      for (j = DAD; j <= MOM; j++)
	{
	  ...
	  pAlleleBits = pPerson->ppNonTransmittedAlleles[j][locus];
  ...
  logMsg (LOGSETRECODING, LOGDEBUG, "Genotype after set recoding:\n");
...
print_person_allele_set (Person * pPerson, int locus, int alleleSetLen)
{
  ...
  if (pPerson->pTypedFlag[locus] == TRUE)
    {
      logMsg (LOGSETRECODING, LOGDEBUG, "    Typed:  ");
      pGenotype = pPerson->ppGenotypeList[locus];
      while (pGenotype != NULL)
	{
  ...
  else
    {
      ...
      logMsg (LOGSETRECODING, LOGDEBUG,
	      "    Paternal Transmitted Alleles: \n");
      print_allele_set (pPerson->ppTransmittedAlleles[DAD][locus],
			alleleSetLen);
      /* maternal transmitted alleles */
      logMsg (LOGSETRECODING, LOGDEBUG,
	      "    Maternal Transmitted Alleles: \n");
      print_allele_set (pPerson->ppTransmittedAlleles[MOM][locus],
			alleleSetLen);
      /* paternal non-transmitted alleles */
      logMsg (LOGSETRECODING, LOGDEBUG,
	      "    Paternal Non-Transmitted Alleles: \n");
      print_allele_set (pPerson->ppNonTransmittedAlleles[DAD][locus],
			alleleSetLen);
      /* maternal non-transmitted alleles */
      logMsg (LOGSETRECODING, LOGDEBUG,
	      "    Maternal Non-Transmitted Alleles: \n");
      print_allele_set (pPerson->ppNonTransmittedAlleles[MOM][locus],
			alleleSetLen);
    }
}

int
print_allele_set (int *pAlleleSet, int len)
{
  int i, j;
  int allele;
  unsigned mask;
...
print_locus_allele_set (Locus * pLocus, int alleleSetLen)
{
  ...
  logMsg (LOGSETRECODING, LOGDEBUG, "Locus %s\n", pLocus->sName);
  for (i = 0; i < pLocus->numAlleleSet; i++)
    {
      logMsg (LOGSETRECODING, LOGDEBUG, "  AlleleSet %d(%f): ", i,
...
construct_original_allele_set_list (int locus)
{
  ...
  for (i = 0; i < numAlleleSet; i++)
	...
	(int *) MALLOC ("pAlleleSet->pAlleles", sizeof (int));
      ...
      pAlleleSet->pAlleleBits = (int *) MALLOC ("pAlleleSet->pAlleleBits",
						sizeof (int) * alleleSetLen);
      ...
      if (modelOptions.polynomial == TRUE)
	{
	  ...
	  pAlleleSet->sumFreqPolynomial =
	    variableExp (&pLocus->pAlleleFrequency[i], vName);
...
int
add_allele_set (int locus, int *pAlleleBits)
{
  Locus *pLocus;
  int numAlleleSet;
  int maxNumAlleleSet;
...
int
find_allele_set (Locus * pLocus, int *pAlleleBits, int alleleSetLen)
{
  int i;
  int j;
  AlleleSet *pAlleleSet;
...
int
is_subset (int *pAlleleBits1, int *pAlleleBits2, int alleleSetLen)
{
  int i;
  for (i = 0; i < alleleSetLen; i++)
    {
