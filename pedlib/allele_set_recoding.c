      ...
      if (modelOptions.polynomial == TRUE)
	{
	  ...
	  sprintf (vName, "f[%d][%d]", locus, i);
         ...
         if(pLocus->locusType==LOCUS_TYPE_MARKER)
             pAlleleSet->sumFreqPolynomial = constantExp(pLocus->pAlleleFrequency[i]);
         else
...
//		  fprintf (stderr, "\n");
                  if(pLocus->locusType==LOCUS_TYPE_MARKER)
                      pAlleleSet->sumFreqPolynomial=plusExp(2,1.0,pAlleleSet->sumFreqPolynomial,
                                1.0,constantExp(pLocus->pAlleleFrequency[allele - 1]));
                  else
		      pAlleleSet->sumFreqPolynomial =
