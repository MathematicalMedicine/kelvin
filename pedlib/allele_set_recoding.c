...
int find_allele_set (Locus * pLocus, unsigned int *pAlleleBits, int alleleSetLen);
int is_subset (unsigned int *pAlleleBits1, unsigned int *pAlleleBits2, int alleleSetLen);
  ...
  for (i = 0; i < numPerson; i++)
      ...
      for (j = DAD; j <= MOM; j++)
	{
	  ...
	  for (m = 1; m <= numAllele; m++)
	    {
	      mask = 1 << ( (m-1) % INT_BITS);
	      pPerson->pNonTransmittedAlleles[j][ (m-1) / INT_BITS] |=
		mask;
	    }
	}
...
print_person_allele_set (Person * pPerson, int locus, int alleleSetLen)
{
  ...
  if (pPerson->pTypedFlag[locus] == TRUE)
  ...
  else
    {
      ...
      KLOG (LOGSETRECODING, LOGDEBUG,
	      "    Paternal Transmitted Alleles: \n");
      print_allele_set (pPerson->pTransmittedAlleles[DAD],
			alleleSetLen);
      /* maternal transmitted alleles */
      KLOG (LOGSETRECODING, LOGDEBUG,
	      "    Maternal Transmitted Alleles: \n");
      print_allele_set (pPerson->pTransmittedAlleles[MOM],
			alleleSetLen);
      /* paternal non-transmitted alleles */
      KLOG (LOGSETRECODING, LOGDEBUG,
	      "    Paternal Non-Transmitted Alleles: \n");
      print_allele_set (pPerson->pNonTransmittedAlleles[DAD],
			alleleSetLen);
      /* maternal non-transmitted alleles */
      KLOG (LOGSETRECODING, LOGDEBUG,
	      "    Maternal Non-Transmitted Alleles: \n");
      print_allele_set (pPerson->pNonTransmittedAlleles[MOM],
			alleleSetLen);
    }
}
...
construct_original_allele_set_list (int locus)
{
  ...
  for (i = 0; i < numAlleleSet; i++)
	...
	(int *) MALLOC ("pAlleleSet->pAlleles", sizeof (int));
      ...
      pAlleleSet->pAlleleBits = (unsigned int *) MALLOC ("pAlleleSet->pAlleleBits",
						sizeof (unsigned int) * alleleSetLen);
...
int
is_subset (unsigned int *pAlleleBits1, unsigned int *pAlleleBits2, int alleleSetLen)
{
  int i;
  for (i = 0; i < alleleSetLen; i++)
    {
