...
pedigree_genotype_elimination (int locus, Pedigree * pPedigree)
{
  ...
  while (doneFlag != TRUE)
	...
	doneFlag = TRUE;
      ...
      //logMsg(LOGGENOELIM, LOGDEBUG, "Pass %d:\n", passNo);
				      ...
				      int parent)
{
	  ...
	  while (pChildGenotype != NULL && elimFlag == TRUE)
	    {
		  ...
		  (locus, parent, pGenotype, pChildGenotype) == TRUE)
  ...
  KASSERT (pParent->ppGenotypeList[locus] != NULL,
	   "Pedigree %s Person %s is not compatible at locus %d.\n",
	   pParent->pPedigree->sPedigreeID, pParent->sID, locus + 1);
  ...
  while (pChildGenotype != NULL)
	     ...
	     elimFlag == FALSE)
	{
	  ...
	  while (pParent2Genotype != NULL && compatibleFlag == FALSE &&
		 elimFlag == FALSE)
	    {
	      if (is_parent_child_genotype_compatible (locus, parent1,
						       pParent1Genotype,
						       pChildGenotype) == TRUE
  ...
  while (pParent1Geno != NULL)
	     ...
	     parent2CompatibleFlag == FALSE)
	{
	  ...
	  for (child = 0; child < pNucFam->numChildren && skipFlag == FALSE;
		...
		{
		  if (is_parent_child_genotype_compatible
		      (locus, parent1, pParent1Geno, pChildGeno) == TRUE
		      && is_parent_child_genotype_compatible (locus, parent2,
							      pParent2Geno,
							      pChildGeno) ==
		      TRUE)
		  ...
		  TRUE)
		{
		  ...
		  for (i = 0; (i < pNucFam->numChildren) && skipFlag == FALSE;
			...
			continue;
		      ...
		      elimFlag = TRUE;
		      pOtherChildGenotype =
			pNucFam->ppChildrenList[i]->ppGenotypeList[locus];
		      while (pOtherChildGenotype != NULL && elimFlag == TRUE)
			{
			  if (is_parent_child_genotype_compatible
			      (locus, parent1, pParent1Genotype,
 ...
 * */
int
is_parent_child_genotype_compatible (int locus, int parent,
				     ...
				     Genotype * pChildGeno)
{
  ...
  int i;
  int *pParentAlleleSet1 = pParentGeno->pAlleleBits[DAD];
  int *pParentAlleleSet2 = pParentGeno->pAlleleBits[MOM];
  int *pChildAlleleSet = pChildGeno->pAlleleBits[parent];
   ...
   * if the child's allele is not compatible with both maternal 
   * and paternal alleles, then return error */
  for (i = 0; i < alleleSetLen; i++)
    {
      if ((*pChildAlleleSet & *pParentAlleleSet1) != *pParentAlleleSet1 &&
	  (*pChildAlleleSet & *pParentAlleleSet2) != *pParentAlleleSet2)
	{
	  /* incompatibility found, return FALSE */
	  return FALSE;
	}
...
is_parent_child_allele_compatible (int alleleSetLen,
				   ...
				   int parent, Genotype * pChildGenotype)
{
  int i;
  int *pChildAlleleSet = pChildGenotype->pAlleleBits[parent];
