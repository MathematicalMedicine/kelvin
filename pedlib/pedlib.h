...
typedef struct ModelType {
  ...
  int analysisType;
  /* whether the chromosome is sex linked or not 
   * assuming we only do one chromosome at a time 
   * for sex chromosome, male is XY and female is XX */
  int sexChromosome;
  /* unknown person ID used in the input pedigree file */
  char *sUnknownPersonID;
  /* whether the polynomial is turned on, default should be on */
  int polynomialFlag;
}ModelType;
#endif
