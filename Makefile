...
#
#CFLGS	+= -g -DNOFLOAT -DDEBUG
CFLGS	+= -g -DDEBUG #-DRADSML
...
# File sets. 
NBIN	= kelvin
LIB	= -lped -lutils -lRADSMM -lcdflib -lgsl -lgslcblas -lm
NLIB	= -lniceapi -lnicecom -lniceaux
SRC	= kelvin.c config.c
INC	= kelvin.h
TAR	= Makefile $(SRC) $(INC) config.c utils pedlib RADSMM flankpick .maj .min .pat .dat doc kelvin.conf
