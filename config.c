 ...
 * lines. 
 **********************************************************************/
/* DIRECTIVE matches: GF, DD, Dd, dd, Th, Tm, Tf, 00..99, AL, MM, AM, TL, TM, TT, LD */
#define DIRECTIVE "^[[:space:]]*[[:digit:]AdDLGT][[:digit:]dDfFhLMmT]"
		...
		ModelRange *modelRange, ModelOptions *modelOptions)
{
  ...
  while (fgets (line, KMAXLINELEN, fp))
	...
	{
	  modelOptions->markerToMarker = TRUE;
	  KLOG(LOGINPUTFILE, LOGDEBUG, "Configuring for marker to marker analysis\n");
	  continue;
	}
      if (strncmp (line, "AM", 2) == 0)
	{
	  modelOptions->adjacentOnly = TRUE;
	  KLOG(LOGINPUTFILE, LOGDEBUG, "Configuring for adjacent marker analysis\n");
	  continue;
	}
      ...
      if (sscanf (line, "OF %s", outfile) == 1)		/* Output file */
	{
      ...
      if (sscanf (line, "AF %s", avghetfile) == 1)	/* Average hetergeneity LR file */
	{
	  KLOG(LOGINPUTFILE, LOGDEBUG, "Configure output file %s\n", avghetfile); 
	  continue;
	}
      if (sscanf (line, "AH %s", avghomofile) == 1)	/* Average homogeneity LR file */
	{
	  KLOG(LOGINPUTFILE, LOGDEBUG, "Configure output file %s\n", avghomofile); 
	  continue;
	}
...
findLambdas (ModelRange *range, int m, int n)
{
  ...
  if (range->nlambdas == range->maxnlambdas)
    {
      range->lambdas = realloc (range->lambdas, (range->maxnlambdas+CHUNKSIZE)*sizeof(LambdaCell));
  ...
  range->lambdas[range->nlambdas].lambda = (double ***) malloc (l * sizeof (double **));
  for (i = 0; i < l; i++)
    {
      range->lambdas[range->nlambdas].lambda[i] = (double **) malloc ((m-1) * sizeof (double *));
      for (j = 0; j < (m-1); j++)
	range->lambdas[range->nlambdas].lambda[i][j] = (double *) malloc ((n-1) * sizeof (double));
    }
  range->nlambdas++;
  ...
  for (i = 0; i < l; i++)
	...
	range->lambdas[range->nlambdas-1].lambda[i][j][k] = 
	  range->dprime[((int) (i/pow(range->ndprime,k)))%range->ndprime];
