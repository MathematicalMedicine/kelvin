 ...
 * lines. 
 **********************************************************************/
/* DIRECTIVE matches: GF, DD, Dd, dd, Th, Tm, Tf, 00..99, AL, TL, TT, LD */
#define DIRECTIVE "^[[:space:]]*[[:digit:]AdDLGT][[:digit:]dDfFhLmT]"
...
#define DIRECTIVES "GFThTmTfALTLTTLDDDDddd"
...
#define LD 7	/* d prime */
#endif
#define DD 8	/* Must be last for numeric directives to work */
#define Dd 9
#define dd 10
		...
		ModelRange *modelRange, ModelOptions *modelOptions)
{
  ...
  while (fgets (line, KMAXLINELEN, fp))
      ...
      if (sscanf (line, "SS %d", &(modelType->numMarkers)) == 1)
	{
	  ...
	  KLOG(LOGINPUTFILE, LOGDEBUG, "Configuring for sex-specific multipoint analysis\n");
	  continue;
	}
      if (sscanf (line, "SA %d", &(modelType->numMarkers)) == 1)
	{
	  ...
	  KLOG(LOGINPUTFILE, LOGDEBUG, "Configuring for sex-averaged multipoint analysis\n");
	  continue;
	}
      if (strncmp (line, "PE", 2) == 0)
	{
	  modelOptions->polynomial = TRUE;	/* Polynomial evaluation */
	  KLOG(LOGINPUTFILE, LOGDEBUG, "Configuring for polynomial evaluation\n"); 
	  continue;
	}
#if FALSE
      /* Analysis modifiers. */
	...
	{
	  /* CHANGE THIS NOT NEEDED */


	  modelOptions->equilibrium = LE;		/* Linkage equilibrium */
	  KLOG(LOGINPUTFILE, LOGDEBUG, "Configuring for linkage equilibrium\n"); 
	  continue;
	}
      if (strncmp (line, "LD", 2) == 0)
	{

	  /* CHANGE THIS TO SET DPRIME */
	...
	{
	  KLOG(LOGINPUTFILE, LOGDEBUG, "Configuring for %d liability classes\n", modelRange->nlclass); 
	  continue;
	}

      if (sscanf (line, "DA %d", &modelRange->nalleles) == 1)	/* Disease alleles */
	{
	  KLOG(LOGINPUTFILE, LOGDEBUG, "Configuring for %d disease alleles\n", 
	       modelRange->nalleles); 
	  continue;
	}
	...
	{
	  KLOG(LOGINPUTFILE, LOGDEBUG, "Resetting affection status values (%g, %g, %g)\n", 
	       modelOptions->affectionStatus[AFFECTION_STATUS_UNKNOWN],
	       modelOptions->affectionStatus[AFFECTION_STATUS_UNAFFECTED],
      ...
      if (strncmp (line, "QT normal", 9) == 0)
	{
	  ...
	  modelOptions->affectionStatus[AFFECTION_STATUS_UNAFFECTED] = -(INFINITY);
	  modelOptions->affectionStatus[AFFECTION_STATUS_AFFECTED] = INFINITY;
	  ...
	  KLOG(LOGINPUTFILE, LOGDEBUG, "Configuring for quantitative traits (normal distribution)\n"); 
	  continue;
	}
      if (sscanf (line, "QT T %d", &a1) == 1)
	{
	  modelType->trait = QT;		/* Quantitative trait */
	  ...
	  modelRange->npardim = 1;
	  KLOG(LOGINPUTFILE, LOGDEBUG, "Configuring for quantitative traits (T distribution)\n"); 
	  continue;
	}
      ...
      else if (regexec (buffer4, line, 6, &match[0], 0) != REG_NOMATCH)
	{
	   ...
	   * frequencies, thetas, or thresholds. Find out what type of
	   * constraint it is by looking it up in DIRECTIVES. */
	  KASSERT(((dir1 = getType (line)) != ERROR),
		  "Bad constraint directive '%s'; aborting.\n", line);
	...
	{
	  /* Look for value specifications. Find out what type of
	   * directive it is by looking it up in DIRECTIVES. */
	  dir1 = getType (line);
	  ...
	  while (TRUE)
	      ...
	      else if (regexec (buffer1, start, 2, &(match[1]), REG_NOTBOL) != REG_NOMATCH)
		{
		  ...
		  if (dir1 == Th || dir1 == Tm || dir1 == Tf)
		    ...
		    addTraitThreshold (modelRange, strtod(start+match[2].rm_so, NULL));
#if FALSE
		  else if (dir1 == LD)
      ...
      else if (regexec (buffer5, line, 1, &match[0], 0) != REG_NOMATCH)
	{
	   ...
	   * parameter it is by parsing the parameter directive (the
	   * +1 ensures we skip over the leading P in, e.g., P1).
	   * Remember, the parameter number read in will be 1-indexed,
	   * and you need to make it 0-indexed first. */
...
#if FALSE
  /* Are there more validation constraints to add here? */
  KASSERT((modelType->trait != DT && modelOptions->dprime),
	  "Linkage disequilibrium only supported for dichotomous traits.\n");
  KASSERT((modelType->type != TP || modelRange->theta),
	  "Theta specification only for two point analysis.\n");
  KASSERT((modelType->type == TP || !modelRange->tthresh),
	  "Trait threshold specification only for multipoint analysis.\n");
...
addTheta (ModelRange *range, int type, double val)
{
  ...
  /* Validate value. */
  KASSERT ((val >= 0 && val <= 0.5+ERROR_MARGIN), "Bad theta value %g; aborting.\n", val);
...
{
  /* Validate value. */
  KASSERT ((val >= 0 && val <= 1.0+ERROR_MARGIN), "Bad alpha value %g; aborting.\n", val);
...
addRange (ModelRange *range, int type, double lo, double hi, double incr)
{
  ...
  /* TODO: check for roundoff issues, like 0, 4, 1 stopping because
   * you get to 4.00001 */
	       ...
	       int op, int a2, int c2, int p2, int disjunct)
{
  ...
  KASSERT((((a1==Tm && a2==Tf) || (a1==Tf && a2==Tm)) || (a1==TT && a2==TT) ||
...
checkPenets (ModelRange *range, int i)
{
  ...
  while (j < constcnt[SIMPLE])
      ...
      if (constraints[SIMPLE][j].a1 == TT || constraints[SIMPLE][j].a2 == TT)
	{
      ...
      else if (constraints[SIMPLE][j].a1 >= DD && constraints[SIMPLE][j].a2 >= DD)
	{
  ...
  while (j < constcnt[CLASSC])
      ...
      else if (constraints[CLASSC][j].alt == FALSE)
	{
	  ...
	  fprintf (stderr, "!%c%c %d %s %c%c %d %s => %g %s %g\n", 
		   ...
		   constraints[CLASSC][i].c2,
		   (constraints[CLASSC][j].alt==TRUE)?"*":"",
		   ((constraints[CLASSC][j].type != TT)?
		    (range->penet[constraints[CLASSC][j].c1-1][constraints[CLASSC][j].a1-DD][i]):
		    (range->tthresh[constraints[CLASSC][j].a1][i]))
		   ((constraints[CLASSC][j].op==EQ)?"==":(constraints[CLASSC][j].op==NE)?"!=":(constraints[CLASSC][j].op==GE)?">=":">"),
		   ((constraints[CLASSC][j].type != TT)?
		    (range->penet[constraints[CLASSC][j].c2-1][constraints[CLASSC][j].a2-DD][i]):
		    (range->tthresh[constraints[CLASSC][j].a2][i])));
#endif
	  return (FALSE);
	}
      j++;
  ...
  if (range->tthresh)
      ...
      for (k = 0; k < j; k++)
	{
	  ...
	  for (m = 0; m < range->nlclass; m++)
	    {
	      range->tthresh[m][range->ntthresh] = tmp0[((int) (k/l))%i];
	      l = l*i;
	    }
