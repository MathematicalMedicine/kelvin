make clean
make CFLAGS="$* -Wall -O -fopenmp -DMEMGRAPH" ADD_LDFLAGS="-ldl"
mv kelvin kelvin-normal

# Set OMP_NUM_THREADS=<something big> for best performance after compiliation of DLs.
make clean
make CFLAGS="$* -Wall -O -fopenmp -DMEMGRAPH -DPOLYUSE_DL" ADD_LDFLAGS="-ldl"
...
make clean
make CFLAGS="$* -Wall -O -DMEMGRAPH -DPOLYUSE_DL -DPOLYCODE_DL -DFAKEEVALUATE" ADD_LDFLAGS="-ldl"
...
# Notice no OpenMP, as it gains no speed and loses us lots of memory.
make clean
make CFLAGS="$* -Wall -O -DMEMGRAPH -DUSE_SSD -DPOLYUSE_DL -DPOLYCODE_DL -DFAKEEVALUATE" ADD_LDFLAGS="-ldl"
mv kelvin kelvin-POLYCODE_DL-FAKEEVALUATE-SSD

# Diagnostic for DLs.
# Notice no OpenMP, as it gains no speed and loses us lots of memory.
make clean
make CFLAGS="$* -Wall -O -DMEMGRAPH -DPOLYUSE_DL -DPOLYCODE_DL -DPOLYCOMP_DL -DPOLYCHECK_DL" ADD_LDFLAGS="-ldl"
mv kelvin kelvin-POLYCHECK_DL

# Build, code and then compile and evaluate DLs. Works fine for small polynomials.
# Notice no OpenMP, as it gains no speed and loses us lots of memory.
make clean
make CFLAGS="$* -Wall -O -DMEMGRAPH -DPOLYUSE_DL -DPOLYCODE_DL -DPOLYCOMP_DL" ADD_LDFLAGS="-ldl"
mv kelvin kelvin-POLYCOMP_DL

# Build, code and then compile and evaluate DLs. Works fine for small polynomials.
# Notice no OpenMP, as it gains no speed and loses us lots of memory.
make clean
make CFLAGS="$* -Wall -O -DMEMGRAPH -DPOLYUSE_DL -DPOLYCODE_DL -DPOLYCOMP_DL -DFAKEEVALUATE" ADD_LDFLAGS="-ldl"
