...
#define checkpt() printf("Checkpoint at line %d of file \"%s\"\n",__LINE__,__FILE__)
...
int dcuhre_(dcuhre_state *s){
  ...
  if(s->ifail !=0){
    ...
    if(s->verbose >0){
      printf("Checking variables is done successfully\n");
    }
  }
...
int dadhre_(dcuhre_state *s){
  ...
  if(s->verbose >0){
    printf("Initializing generators, weights...\n");
  }
  dinhre_(s);

  if(s->verbose >1){
    printf("Initial generators, weights, rulpts,..");
    print_rule(s);
  }
  if(s->restar ==1){
    printf("This case, restar ==1, is not supported for now\n");
    exit(0);
  }
  ...
  if(s->verbose >0){
    printf("Apply DRLHRE over the whole region\n");
  }
  drlhre_(s, cw_sbrg);
  //printf("end fo first\n");
  s->total_neval += s->num;
  ...
  if(s->verbose >0){
    printf("After %d sub regions result=%20.18f error=%20.18f\n", s->sbrgns,s->result, s->error);
  }
  ...
  //if(s->mType == MT_DT){
    ...
    s->epsabs *= (-5.77 + 54.0* 6*s->result + 36*s->result*s->result)*(-5.77 + 54.0* 6*s->result + 36*s->result*s->result);
    s->epsabs /= (-11.54 * 6*s->result + 54.0*36*s->result*s->result);
	s->epsabs /= 6.0;  /* because of the constraint */
    //s->epsrel = s->epsabs / (s->result);
    printf("Setting absolute error %12.8f \n", s-> epsabs);
  ...
  while(s->sbrgns  < s->maxsub){
    ...
    /*   If MAXSUB is great enough, NDIV = MDIV */
      if(s->verbose >0){
        printf("Select %d subregion with %f error and direction = %d\n", s->next_sbrg,s->greate, s->sbrg_heap[s->next_sbrg]->dir);
      }
      ...
      s->error += cw_sbrg->local_error;
      if(s->verbose >0){ 
        printf("After %d sub regions result=%10.8f error=%10.8f\n", s->sbrgns,s->result, s->error);
      }
      s->sbrgns ++;
	  ...
	  tmp_result = s->result;	  
      if(s->verbose >0){
        printf("After %d sub regions result=%10.8f error=%10.8f and difference=%10.8f\n", s->sbrgns,s->result, s->error,s->diff_result[s->sbrgns]);
      }
  ...
  difmax = 0.;
	...
	difsum = 0.;
        ...
        frthdf = (1 - ratio) * 2 *cw_sbrg->local_error - (null[6] + 
		    null[7]) + ratio * (null[4] + null[5]);

        //printf("printing null 4 %f %f %f %f %f\n", null[4],null[5],null[6],null[7],frthdf);
	...
	cw_sbrg->local_result += s->w[0][1] * (null[4] + null[5]) + s->w[0][2] *(null[6] + null[7]);
    ...
    //printf("null is %10.8f  %10.8f  %10.8f \n", null[0], null[1], null[2]);
   /* Finish computing the rule values. */
    ...
    //printf("local result =%10.8f  and local error =%10.8f\n", cw_sbrg->local_result,cw_sbrg->local_error);
    /*    Compute errors. */
    ...
    /* Computing MAX */
	d__1 = null[0], d__2 = null[1], 
		    d__1 = max(d__1,d__2), d__2 = null[2];
    ...
    *fulsms += *funvls;
	...
	if (s->g[i - 2][g_work_col] > s->g[i-1][g_work_col]) {
		...
		if (s->g[l-1][g_work_col] > gi) {
		  //printf("lxchng =%d is changing to l=%d\n",lxchng,l);
		    lxchng = l;
		}
		// printf("     i=%d  l=%d  IXCHNG =%d, lxchng =%d\n", i, l, ixchng, lxchng);
	    }
...
int d09hre_(dcuhre_state *s){
    ...
    lam3 = (7-(lam2 + lam1)*9 + lam2 *63* lam1 /5 - ratio *63)/(9-(lam2 + lam1)*63/5 + lam2 *21* lam1 - ratio *63 / lam0);
    lamp = .0625f;

    //printf("lam0 = %10.7f  %10.7f %10.7f %10.7f %10.7f \n", lam0,lam1,lam2,lam3,lamp);
/*     Compute degree 9 rule weights */
    ...
    s->w[0][6] = (1 - (lam0 + lam1) * 7 / 5 + lam0 * 7 * lam1 / 3) / (lam1 * 84 * 
	    lam2 * (lam2 - lam0) * (lam2 - lam1));
    s->w[0][5] = (1 - (lam0 + lam2) * 7 / 5 + lam0 * 7 * lam2 / 3) / (lam1 * 84 * 
	    lam1 * (lam1 - lam0) * (lam1 - lam2)) - s->w[0][6] * lam2 / lam1 - 2*(s->ndim - 2) * s->w[0][7];
    s->w[0][3] = (1 - ((lam0 + lam1 + lam2) / 7 - (lam0 * lam1 + lam0 * lam2 + 
	    lam1 * lam2) / 5) * 9 - lam0 * 3 * lam1 * lam2) / (lam3 * 18 * (
	    lam3 - lam0) * (lam3 - lam1) * (lam3 - lam2));
    s->w[0][2] = (1 - ((lam0 + lam1 + lam3) / 7 - (lam0 * lam1 + lam0 * lam3 + 
	    lam1 * lam3) / 5) * 9 - lam0 * 3 * lam1 * lam3) / (lam2 * 18 * (
	    lam2 - lam0) * (lam2 - lam1) * (lam2 - lam3)) - 2*(s->ndim - 1) *
	     s->w[0][6];
    s->w[0][1] = (1 - ((lam0 + lam2 + lam3) / 7 - (lam0 * lam2 + lam0 * lam3 + 
	    lam2 * lam3) / 5) * 9 - lam0 * 3 * lam2 * lam3) / (lam1 * 18 * (
	    lam1 - lam0) * (lam1 - lam2) * (lam1 - lam3)) - 2*(s->ndim - 1) *
	     (s->w[0][6] + s->w[0][5] + (s->ndim - 2) * s->w[0][7]);
...
/*     Compute weights for 2 degree 7, 1 degree 5 and 1 degree 3 rules */
    ...
    s->w[1][6] = (1- lam1 *5/3 - twondm *15* s->w[1][s->wtleng-1] *(lam0*lam0) * 
                 (lam0 - lam1)) / (lam1 *60* lam2 * (lam2 - lam1));
    s->w[1][5] = (1 - 9*(8* lam1 * lam2 * s->w[1][6] + twondm * s->w[1][s->wtleng -1] * lam0*lam0)) / 
                 (lam1 * 36 * lam1) - 2* s->w[1][7]  * (s->ndim - 2);
    s->w[1][3] = (1 - 7*((lam1 + lam2) / 5 - lam1 * lam2 / 3 + twondm * s->w[1][s->wtleng-1] * lam0 * 
                 (lam0 - lam1) * (lam0 - lam2)))/(14* lam3 *(lam3 - lam1) * (lam3 - lam2));
    s->w[1][2] = (1 - 7*((lam1 + lam3)/5 - lam1 * lam3 / 3 + twondm * s->w[1][s->wtleng-1] * 
                 lam0 * (lam0 - lam1) * (lam0 - lam3)))/(lam2 * 14 *
	     (lam2 - lam1) * (lam2 - lam3)) - 2*(s->ndim - 1) * s->w[1][6];
    s->w[1][1] = (1 - 7*((lam2 + lam3) / 5 - lam2 * lam3 / 3 + twondm * s->w[1][s->wtleng -1] * lam0 * 
                 (lam0 - lam2) * (lam0 - lam3))) / (lam1 *14*(lam1 - lam2) * 
                 (lam1 - lam3)) - 2*(s->ndim - 1)*(s->w[1][6] + s->w[1][5] + (s->ndim - 2) * s->w[1][7]);    

    s->w[2][s->wtleng -1]= 5/(324* lam0* lam0 * lam0 * lam0)/twondm;
    if(s->ndim >2)
      s->w[2][7] = (1-27*twondm * s->w[2][8]* lam0 * lam0 * lam0)/d;

    s->w[2][6] = (1 - lam1 * 5 / 3 - twondm * 15 * s->w[2][s->wtleng -1] * (lam0 * lam0) * 
                 (lam0 - lam1)) / (lam1 * 60 * lam2 * (lam2 - lam1));
    s->w[2][5] = (1 - 9*(lam1 * 8 * lam2 * s->w[2][6] + twondm * s->w[2][s->wtleng -1] * lam0 * lam0)) / 
                 (lam1 * 36 * lam1) - s->w[2][7] * 2 * (s->ndim - 2);
    s->w[2][4] = (1 - 7*((lam1 + lam2) / 5 - lam1 * lam2 / 3 + twondm * s->w[2][s->wtleng -1] * lam0 * 
                 (lam0 - lam1) * (lam0 - lam2))) / (lamp * 14 *(lamp - lam1) * (lamp - lam2));
    s->w[2][2] = (1 - 7*((lam1 + lamp) / 5 - lam1 * lamp / 3 + twondm * s->w[2][s->wtleng -1] * lam0 * 
                 (lam0 - lam1) * (lam0 - lamp))) / (lam2 * 14 *(lam2 - lam1) * (lam2 - lamp)) - 2*(s->ndim - 1) * s->w[2][6];
    s->w[2][1] = (1 - 7*((lam2 + lamp) / 5 - lam2 * lamp / 3 + twondm * s->w[2][s->wtleng -1] * lam0 * 
                 (lam0 - lam2) * (lam0 - lamp))) / (lam1 * 14 *(lam1 - lam2) * (lam1 - lamp)) - 2*(s->ndim - 1) * 
                 (s->w[2][6] + s->w[2][5] + (s->ndim - 2) * s->w[2][7]);

    s->w[3][s->wtleng -1]= 2/(81* lam0 * lam0 * lam0 * lam0)/twondm;
    if(s->ndim >2)
      s->w[3][7] = (2-27*twondm * s->w[3][8]* lam0 * lam0 * lam0)/d;
    
    s->w[3][6] = (2 - lam1 * 15 / 9 - twondm * 15 * s->w[3][s->wtleng -1] * lam0 * 
                 (lam0 - lam1)) / (lam1 * 60 * lam2 * (lam2 - lam1));
    s->w[3][5] = (1 - 9*(lam1 * 8 * lam2 * s->w[3][6] + twondm * s->w[3][s->wtleng -1] * 
		 (lam0 * lam0))) / (lam1 * 36 * lam1) - s->w[3][7] * 2 * (s->ndim - 2);
    s->w[3][3] = (2 - 7*((lam1 + lam2) / 5 - lam1 * lam2 / 3 + twondm * s->w[3][s->wtleng -1] * lam0 * 
                 (lam0 - lam1) * (lam0 - lam2))) / (lam3 * 14 *(lam3 - lam1) * (lam3 - lam2));
    s->w[3][2] = (2 - 7*((lam1 + lam3) / 5 - lam1 * lam3 / 3 + twondm * s->w[3][s->wtleng -1] * lam0 * 
                 (lam0 - lam1) * (lam0 - lam3)))/(lam2 *14*(lam2 - lam1)*(lam2 - lam3))-2*(s->ndim -1) * s->w[3][6];
    s->w[3][1] = (2 - 7*((lam2 + lam3) / 5 - lam2 * lam3 / 3 + twondm * s->w[3][s->wtleng -1] * lam0 * 
                 (lam0 - lam2) * (lam0 - lam3))) / (lam1 *14*(lam1 - lam2) * (lam1 - lam3)) - 2*(s->ndim - 1) * 
                 (s->w[3][6] + s->w[3][5] + (s->ndim - 2) * s->w[3][7]);
    s->w[4][1] = 1 / (lam1 * 6);
...
/*     Compute degree 7 rule weights */
    ...
    s->w[0][2] = (1 - lam2 * 5 / 3 - twondm * 5 * s->w[0][5] * lam0 * (lam0 - lam2)) / 
                 (lam1 * 10 * (lam1 - lam2)) - 2*(s->ndim - 1) * s->w[0][4];
    s->w[0][1] = (1 - lam1 * 5 / 3 - twondm * 5 * s->w[0][5] * lam0 * (lam0 - lam1)) / 
                 (lam2 * 10 * (lam2 - lam1));

/*     Compute weights for 2 degree 5, 1 degree 3 and 1 degree 1 rules */

    s->w[1][5] = 1 / ( lam0 * lam0 * lam0 * 36) / twondm;
    s->w[1][4] = (1 - twondm * 9 * s->w[1][5] * lam0 * lam0) / (lam1 * lam1 * 36);
    s->w[1][2] = (1 - lam2 * 5 / 3 - twondm * 5 * s->w[1][5] * lam0 * (lam0 - lam2)) / 
                 (lam1 * 10 * (lam1 - lam2)) - 2*(s->ndim - 1) * s->w[1][4];
    s->w[1][1] = (1 - lam1 * 5 / 3 - twondm * 5 * s->w[1][5] * lam0 * (lam0 - lam1)) / 
                 (lam2 * 10 * (lam2 - lam1));
    s->w[2][5] = 5 / (lam0 * lam0 * lam0 * 108) / twondm;
    s->w[2][4] = (1 - twondm * 9 * s->w[2][5] * lam0 * lam0) / (lam1 * lam1 * 36);
    s->w[2][2] = (1 - lamp * 5 / 3 - twondm * 5 * s->w[2][5] * lam0 * (lam0 - lamp)) / 
                 (lam1 * 10 * (lam1 - lamp)) - 2*(s->ndim - 1) * s->w[2][4];
    s->w[2][3] = (1 - lam1 * 5 / 3 - twondm * 5 * s->w[2][5] * lam0 * (lam0 - lam1)) / 
                 (lamp * 10 * (lamp - lam1));
    s->w[3][5] = 1 / (lam0 * lam0 * lam0 * 54) / twondm;
    s->w[3][4] = (1 - twondm * 18 * s->w[3][5] * lam0 * lam0) / (lam1 * lam1 * 72);
    s->w[3][2] = (1 - lam2 * 10 / 3 - twondm * 10 * s->w[3][5] * lam0 * (lam0 - lam2)) /
	         (lam1 * 20 * (lam1 - lam2)) - 2*(s->ndim - 1) * s->w[3][4];
    s->w[3][1] = (1 - lam1 * 10 / 3 - twondm * 10 * s->w[3][5] * lam0 * (lam0 - lam1)) /
	         (lam2 * 20 * (lam2 - lam1));
...
int dchhre_(dcuhre_state *s){
  ...
  if (s->keyf == 1) {
    ...
    s->num = 1+4*2*s->ndim + 2*s->ndim*(s->ndim-1) + 4*s->ndim*(s->ndim-1)+ 4*s->ndim*(s->ndim-1)*(s->ndim-2)/3 + pow_i(2,s->ndim);
    s->wtleng = 9;
    if (s->ndim == 2) {
      s->wtleng = 8;
    }
  } else if (s->keyf == 4) {
    s->num = 1+s->ndim *6 + (s->ndim*2)*(s->ndim-1) + pow_i(2,s->ndim);
    s->wtleng = 6;
  }
...
int initialize_state(dcuhre_state *s,double *a, double *b, int dim){
...
int sbrg_free(sub_region ** sbrg_heap, int maxsub){
...
int print_rule(dcuhre_state *s){
  ...
  printf("Number of function evaluation for each subregion is %d\n", s->num);
  printf("Printing g whose size is dim=%d x wtleng =%d\n", s->ndim, s->wtleng);  
  for(i=0;i<s->ndim;i++){
    for(j=0;j< s->wtleng; j++)
      printf(" %f", s->g[i][j]);
    printf("\n");
  ...
  printf("Printing w whose size is 5 x wtleng =%d\n",  s->wtleng);  
  for(i=0;i<5;i++){
    for(j=0;j< s->wtleng; j++)
      printf(" %f", s->w[i][j]);
    printf("\n");
  ...
  printf("Printing rulpts whose size is  wtleng =%d\n",  s->wtleng);
  for(j=0;j< s->wtleng; j++)
    printf(" %d", s->rulpts[j]);
  printf("\n");


  printf("Printing scales whose size is 3 x wtleng =%d\n",  s->wtleng);  
  for(i=0;i<3;i++){
    for(j=0;j< s->wtleng; j++)
      printf(" %f", s->scales[i][j]);
    printf("\n");
  }

  printf("Printing norms whose size is s x wtleng =%d\n",  s->wtleng);  
  for(i=0;i<3;i++){
    for(j=0;j< s->wtleng; j++)
      printf(" %f", s->norms[i][j]);
    printf("\n");
  }


  printf("Printing rulpts whose size is 6\n");
  for(j=0;j< 6; j++)
    printf(" %f", s->errcof[j]);
  printf("\n");
...
int print_sbrg(sub_region *cw_sbrg, int dim){
  int i;
  printf("Printing subregion with id = %d\n", cw_sbrg->region_id);
  ...
  printf("on the sub region center   hwidth\n");
  for(i=0; i<dim;i++){
    printf("                 %f, %f\n",cw_sbrg->center[i], cw_sbrg->hwidth[i]);
  }
