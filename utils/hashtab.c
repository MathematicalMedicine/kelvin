     ...
     htab *t;			/* table */
{
  ...
  printf ("warning: hash table size doubling to %d\n", (int) newsize);
     ...
     word logsize;		/* log base 2 of the size of the hash table */
{
  ...
  len = ((ub4) 1 << logsize);
  fprintf (stderr,
	   "info: creating hashtable of size %d from log base 2 of %d...",
	   (int) len, logsize);
     ...
     htab *t;
{
  ...
  for (i = 0; i <= t->mask; ++i) {
	...
	for (walk2 = stat;
	     walk2->next && (walk2->next->keyl < j); walk2 = walk2->next);
