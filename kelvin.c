...
double compute_hlod_2p_qt(int *ndim, double x[], double *f );
double compute_hlod_mp_qt(int *ndim, double x[], double *f );
...
/* End of Adding from kelvin-0.33.1*/
 ...
 // fphlod = fopen("hlod.pts","w");	 
  ...
  if (modelOptions.polynomial == TRUE){
	for (k = 0; k < 3; k++){
	  initialProbPoly[k] = constant1Poly;//constantExp (1.0);
	  initialProbPoly2[k] = constant1Poly;//constantExp (1.0);
		...
		for (mkrFreqIdx = 0;mkrFreqIdx == 0 || mkrFreqIdx < modelRange.nafreq;mkrFreqIdx++){
		  ...
		  if (modelType.trait == DICHOTOMOUS){
			...
			fprintf (fpHet,"%6s %6s %8s %8s %8s %6s %6s %5s %5s %5s \n",
		       "Theta",
		       "COUNT", "BR",  "ERR_EST",  "MAX_HLOD",  "ALPHA", "DGF",
	        ...
	        for(i=0;i<140;i++){
			 ...
			 // fprintf(fpSeok_theta,"%f %f ",	fixed_dprime, fixed_theta);
              ...
              fprintf(fpHet,"%6.4f %6.4f %6d %8.4f %8.4f %8.4f %6.4f %6.4f  ",fixed_dprime, fixed_theta, s->total_neval, integral, abserr, log10(localmax_value), localmax_x[1],localmax_x[0]);
			  for (liabIdx = 0; liabIdx < modelRange.nlclass; liabIdx++){
			    fprintf(fpHet,"%6.4f %6.4f %6.4f", localmax_x[liabIdx*3+2],localmax_x[liabIdx*3+3],localmax_x[liabIdx*3+4]);
			  }
			  fprintf(fpHet,"\n");
...
*/
			...
			fprintf(fpPPL," %8.4f %6.4f %6.4f %6.4f %6.4f ",log10(maximum_function_value), maxima_x[0],maxima_x[1],maxima_x[3],maxima_x[2]);	
			for (liabIdx = 0; liabIdx < modelRange.nlclass; liabIdx++){
			  fprintf(fpPPL,"%6.4f %6.4f %6.4f ", maxima_x[liabIdx*3 +4],maxima_x[liabIdx*3 +5],maxima_x[liabIdx*3 +6]);			  
			}
	        fprintf (fpPPL, "\n");
	        fflush (fpPPL);							
	...
	fprintf (fpHet,"           pos       PPL         BR       error    num    markerList   maximum   alpha    gf     DD       Dd      dd\n");
	prevFirstMarker = -1;
	prevLastMarker = -1;
	prevTraitInd = -1;
	leftMarker = -1;
	for (posIdx = 0; posIdx < numPositions; posIdx++){
	  ...
	  fprintf (fpHet, ") %f %f %f ", log10(localmax_value), localmax_x[1],localmax_x[0]);
	  for (liabIdx = 0; liabIdx < modelRange.nlclass; liabIdx++){
	    fprintf (fpHet, " %f %f %f ",localmax_x[3*liabIdx +2],localmax_x[3*liabIdx +3],localmax_x[3*liabIdx +4]);
	  }
	  fprintf(fpHet, "\n");
	  fflush (fpHet);
  ...
  for (j = 0; j < 5; j++){
    ...
    if(hetLR > localmax_value){
      ...
      for (liabIdx = 0; liabIdx < modelRange.nlclass; liabIdx++){
		...
		localmax_x[3*liabIdx+4] = x[3*liabIdx+3]*x[3*liabIdx+1]*x[3*liabIdx+2];	 
	  } 
	}	 	
