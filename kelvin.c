...
*/
			  ...
			  if(modelType.trait != DT)
			    {
			      ...
			      fprintf (fpHet, " %5.3f %5.3f %5.3f %5.3f ", SD_DD,
				       SD_Dd, SD_dd, threshold);
			    }
			}
		      fprintf (fpHet, "\n");
		      fflush (fpHet);
      ...
      if (modelOptions.polynomial == TRUE)
	{
	  ...
	  /* populate the matrix */
	  status = populate_xmission_matrix (altMatrix, totalLoci, 
	  ...
	  xmissionMatrix = nullMatrix;
	  /* populate the matrix */
	  status = populate_xmission_matrix (nullMatrix, totalLoci, 
					     initialProbAddr,	/* probability */
					     0,	/* cell index */
					     -1,	/* last he locus */
					     -1,	/* last het pattern (P-1 or M-2) */
