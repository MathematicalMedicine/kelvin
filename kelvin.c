				      ...
				      for (j = 0; j < modelRange.nalpha; j++)
					{
					  ...
					  for (pedIdx = 0;
						...
						nullLikelihood[pedIdx];
					      tmp = log10 (alphaV * homoLR +
							   (1 - alphaV));

					      log10HetLR +=
						tmp * pPedigree->pCount[loc2];
				   ...
				   penIdx++)
				{
				  ...
				  for (thresholdIdx = 0;
					  ...
					  LINKAGE_EQUILIBRIUM)
					{
					  ...
					  copy_DValue (pLDLoci, pLambdaCell->
							  DValue[dprime0Idx]);
      ...
      if (pTrait->type == DICHOTOMOUS)
	{
	  for (penIdx = 0; penIdx < modelRange.npenet; penIdx++)
		   ...
		   gfreqInd++)
		{
		  ...
		  likelihoodDT[pedigreeSet.numPedigree][gfreqInd][penIdx] =
		    log10_likelihood_null;
		}	/* gfreq */
	    }		/* pen */
	}
		  ...
		  for (paramIdx = 0; paramIdx < modelRange.nparam; paramIdx++)
		      ...
		      for (penIdx = 0; penIdx < modelRange.npenet; penIdx++)
			{
			  ...
			  for (thresholdIdx = 0;
				...
				status = populate_xmission_matrix (xmissionMatrix, 
								   totalLoci, 
								   initialProbAddr, /* probability */
								   initialProbAddr2, /* probability */
								   initialHetProbAddr, 
			      ...
			      for (j = 0; j < modelRange.nalpha; j++)
				{
				  ...
				  for (pedIdx = 0;
					...
					pPedigree->likelihood /
					(likelihoodQT[pedIdx][gfreqInd][penIdx]
					[paramIdx][thresholdIdx] * pPedigree->markerLikelihood);
				      log10HetLR +=
				  ...
				  if (log10HetLR >= DBL_MAX_10_EXP - 1)
				    ...
				    if (log10HetLR <= DBL_MIN_10_EXP + 1)
				    {
				      hetLR = 0;
				    }
				  else
