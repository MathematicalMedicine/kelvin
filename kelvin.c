...
double calculate_PPL(SUMMARY_STAT *result, 
		     double thetaCutoff, double weight,
		     double prior);
			  ...
			  for (dprimeIdx = 0;
				  ...
				  LINKAGE_EQUILIBRIUM)
				{
				  ...
				  if(R_square_flag == TRUE)
				    R_square = calculate_R_square(pLocus1->pAlleleFrequency[0], 
								  pLocus2->pAlleleFrequency[0],
								  pLDLoci->ppDValue[0][0]);
				  else
				    R_square = -1;

				  if(-ERROR_MARGIN <= pLambdaCell->lambda[dprimeIdx][0][0] && pLambdaCell->lambda[dprimeIdx][0][0] <= ERROR_MARGIN)
				   ...
				   thetaInd++)
				{
				  ...
				  for (j = 0; j < modelRange.nalpha; j++)
					  ...
					  max_lr)
					{
					  ...
					  tp_result[dprimeIdx][thetaInd].
					    max_penIdx = penIdx;
					  tp_result[dprimeIdx][thetaInd].R_square = R_square;
					}
...
*/
	      ...
	      for (thetaInd = 0; thetaInd < modelRange.ntheta; thetaInd++)
		{
	      ...
	      ppl = calculate_PPL(tp_result[dprime0Idx], thetaCutoff, thetaWeight, prior);
	      ...
	      if (modelOptions.equilibrium != LINKAGE_EQUILIBRIUM)
		{
		  ...
		  get_average_LD_LR(tp_result, pLambdaCell->ndprime, modelRange.ntheta);
		  ppl = calculate_PPL(tp_result[pLambdaCell->ndprime], thetaCutoff, thetaWeight, prior);
...
void free_likelihood_storage(PedigreeSet *pedSet)
{
  int pedIdx;
  int gfreqInd;
  int penIdx;
		     ...
		     double prior)
{
  ...
  for(i=0; i < numTheta -1; i++)
	...
	{
	  a = avgLR1 + (avgLR2 - avgLR1)/ (theta2 - theta1) * (thetaCutoff - theta1);
