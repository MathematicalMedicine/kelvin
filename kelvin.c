			   ...
			   thetaInd++)
			{
			  ...
			  for (j = 0; j < modelRange.nalpha; j++)
				   ...
				   pedIdx < pedigreeSet.numPedigree; pedIdx++)
				{
			      ...
			      pedigreeSet.log10HetLR[j] = log10HetLR;
...
*/
			   ...
			   dprimeIdx++)
			{
			  ...
			  for (thetaInd = 0; thetaInd < modelRange.ntheta;
			      ...
			      for (j = 0; j < modelRange.nalpha; j++)
				{
				  ...
				  pedigreeSet.log10HetLR[j] = hetLR;
				  hetLR = pow (10, hetLR);
				  pedigreeSet.hetLR[j] = hetLR;
	  ...
	  for (thetaInd = 0; thetaInd < modelRange.ntheta; thetaInd++)
	      ...
	      if (modelOptions.equilibrium == LINKAGE_EQUILIBRIUM)
		{
		  fprintf (stderr, "\t (%f,%f)  %12.8f(%d)\n",
			   theta, theta,
			   tp_result[dprimeIdx][thetaInd].het_lr_total /
			   (modelRange.nalpha * tp_result[dprimeIdx][thetaInd].lr_count),
			   modelRange.nalpha * tp_result[dprimeIdx][thetaInd].lr_count);
		}
	      else
		{
    ...
    {
      /* multipoint */
      locusList.maxNumLocus = modelType.numMarkers + 1;
      locusList.pLocusIndex =
	(int *) calloc (locusList.maxNumLocus, sizeof (int));
      locusList.pPrevLocusDistance =
	(double *) calloc (locusList.maxNumLocus, sizeof (double));
      locusList.pNextLocusDistance =
	(double *) calloc (locusList.maxNumLocus, sizeof (double));
      ...
      for (posIdx = 0; posIdx < numPositions; posIdx++)
	{
	  ...
	  /* save the analysis locus list */
	  memcpy (&savedLocusList, &locusList, sizeof (locusList));

	  /* store the markers used */
	  mp_result[posIdx].pMarkers =
	    (int *) calloc (modelType.numMarkers, sizeof (int));
	  k = 0;		/* marker index */
	  for (i = 0; i <= modelType.numMarkers; i++)
	    {
	      ...
	      for (penIdx = 0; penIdx < modelRange.npenet; penIdx++)
		{
		  ...
		  for (gfreqInd = 0; gfreqInd < modelRange.ngfreq; gfreqInd++)
			...
			update_locus (&pedigreeSet, traitLocus);
		      ...
		      memcpy (&locusList, &nullLocusList, sizeof (locusList));
			   ...
			   pedIdx++)
			{
		      ...
		      memcpy (&locusList, &savedLocusList,
			      sizeof (locusList));
		      compute_likelihood (&pedigreeSet);
		      ...
		      for (penIdx = 0; penIdx < modelRange.npenet; penIdx++)
			{
			  ...
			  for (thresholdIdx = 0;
				...
				continue;
			      update_penetrance (&pedigreeSet, traitLocus);
			      /* get the likelihood at NULL hypothesis - use nullLocusList */
			      memcpy (&locusList, &nullLocusList,
				      sizeof (locusList));
