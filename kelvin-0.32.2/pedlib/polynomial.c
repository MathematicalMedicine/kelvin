	  ...
	  for (i = first; i <= last; i++)
	    {
	      ...
	      if ((vType == 'D'
		   && variableList[vIndex]->e.v->vAddr.vAddrD == vD)
		  || (vType == 'I'
	      ...
	      if (counterSum == sumList[sIndex]->e.s->num)
		{
		  ...
		  for (k = 0; k < counterSum; k++)
		      ...
		      if (pSum[k] == sumList[sIndex]->e.s->sum[k])
			{
			  ...
			  tempD2 =
			    frexp (sumList[sIndex]->e.s->factor[k], &tempI2);
			  if (tempI2 != tempI
...
timesExp (int num, ...)
{
  ...
  if (counterProd > lengthProd)
    {
      ...
      exponentProd =
	(int *) realloc (exponentProd, lengthProd * sizeof (int));
      pProd =
  ...
  //The result polynomial is a product polynomial
	  ...
	  if (p0HIndex != hIndex || p0SubHIndex != location)
	      ...
	      else		//p0SubHIndex!=binaryEnd
		{
		  ...
		  if (p0SubHIndex < location)
		    ...
		    {
		      deleteHashTable (&productHash[p0HIndex],
				       p0SubHIndex + 1);
		    }
		}
	    }
	...
	//in the hash table
	{
	  insertHashTable (&productHash[hIndex], location, key,
			   productCount - 1);
	}
...
functionCallExp (int num, ...)
{
  ...
  if (functionCallCount >= functionCallListLength)
	...
	{
	  fprintf (stderr,
		   "Memory allocation error in functionCallExp, exit!");
	  exit (1);
	}
    }
  ...
  if (l->listNext >= l->listSize)
    {
      l->pList =
	realloc (l->pList,
		 sizeof (struct polynomial *) * (l->listSize + 1000));
      if (l->pList == NULL)
	{
	  fprintf (stderr, "Memory allocation error, exit!\n");
	  exit (1);
...
//             p->value*=pow(p->e.p->product[i]->value,p->e.p->exponent[i]);
	...
	case T_FUNCTIONCALL:
	  if (strcmp (p->e.f->name, "log10") == 0)
	    ...
	    {
	      p->value =
		gsl_cdf_chisq_P (p->e.f->para[0]->value,
				 p->e.f->para[1]->value);
	    }
	  else if (strcmp (p->e.f->name, "gsl_cdf_chisq_Q") == 0)
	    {
	      p->value =
		gsl_cdf_chisq_Q (p->e.f->para[0]->value,
				 p->e.f->para[1]->value);
	    }
	  else if (strcmp (p->e.f->name, "gsl_ran_chisq_pdf") == 0)
	    {
	      p->value =
		gsl_ran_chisq_pdf (p->e.f->para[0]->value,
				   p->e.f->para[1]->value);
	    }
	  ...
	  for (i = 0; i < p->e.p->num; i++)
	    {
	      fprintf (stderr,
		       ...
		       p->e.p->product[i]->value, p->e.p->exponent[i],
		       p->value);
	    }
	  break;
...
polynomialInitialization ()
{
  ...
  for (i = 0; i < FUNCTIONCALL_HASH_SIZE; i++)
	...
	(int *) malloc (functionCallHash[i].length * sizeof (int));
      if (functionCallHash[i].index == NULL
...
//   fprintf(stderr,"productList clearance completed!\n");
  ...
  for (j = functionCallCountStamp2; j < functionCallCount; j++)
      ...
      if (functionCallList[j]->valid >= 1)
	{
	  ...
	  functionCallIndex[j - functionCallCountStamp2] =
	    functionCallRecount;
	  functionCallRecount++;
	}
      else
  ...
  for (j = 0; j < PRODUCT_HASH_SIZE; j++)
      ...
      if (productHash[j].num > 0)
	{
	  ...
	  for (i = 0; i < productHash[j].num; i++)
		...
		if (productIndex[productHash[j].index[i] - productCountStamp2]
		    != -1)
		{
		  productHash[j].index[k] =
		    productIndex[productHash[j].index[i] -
				 productCountStamp2];
		  productHash[j].key[k] = productHash[j].key[i];
		  k++;
		}
...
polyStatistics (FILE * fp)
{
  ...
  fprintf (stderr, "Number of polys:  %8d  %3d  %11d  %11d  %5d (%d %d)\n",
	   constantCount, variableCount, sumCount, productCount,
	   functionCallCount, sumTerms, productTerms);
  fprintf (stderr,
	   "sizes of polys:   %8ld  %3ld  %11ld  %11ld  %5ld  %12ld\n",
  ...
  if (fp != NULL)
    fprintf (fp, " %10ld   %10ld   %10ld   %10ld   %10ld   %10ld (%d  %d)\n",
	     ...
	     functionCallSize,
	     constantSize + variableSize + sumSize + productSize +
	     functionCallSize, sumTerms, productTerms);
...
printAllPolynomials ()
{
  ...
  if (constantCount > 0)
      ...
      for (i = 0; i < CONSTANT_HASH_SIZE; i++)
	{
	  ...
	  for (j = 0; j < constantHash[i].num; j++)
	    {
	      fprintf (stderr, "(%d  %d) index=%d key=%d valid=%d constant: ",
		       i, j, constantHash[i].index[j], constantHash[i].key[j],
  ...
  if (variableCount > 0)
      ...
      for (i = 0; i < VARIABLE_HASH_SIZE; i++)
	{
	  ...
	  for (j = 0; j < variableHash[i].num; j++)
	    {
	      fprintf (stderr, "(%d  %d) index=%d key=%d variable: ", i, j,
  ...
  if (sumCount > 0)
      ...
      for (i = 0; i < SUM_HASH_SIZE; i++)
	{
	  ...
	  for (j = 0; j < sumHash[i].num; j++)
	    {
	      fprintf (stderr, "(%d  %d) index=%d Key=%d sum: ", i, j,
		       sumHash[i].index[j], sumHash[i].key[j]);
  ...
  if (productCount > 0)
      ...
      for (i = 0; i < PRODUCT_HASH_SIZE; i++)
	{
	  ...
	  for (j = 0; j < productHash[i].num; j++)
	    {
	      fprintf (stderr, "(%d  %d) index=%d Key=%d product: ", i, j,
  ...
  if (functionCallCount > 0)
      ...
      for (i = 0; i < FUNCTIONCALL_HASH_SIZE; i++)
	{
	  ...
	  for (j = 0; j < functionCallHash[i].num; j++)
	    {
	      fprintf (stderr, "(%d  %d) index=%d Key=%d functionCall: ", i,
		       j, functionCallHash[i].index[j],
		       functionCallHash[i].key[j]);
