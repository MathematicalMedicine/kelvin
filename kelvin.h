...
typedef struct modelType
{
  ...
  double min;                   /* QT - minimum value of QT - left truncate */
  double max;                   /* QT - maximum value of QT - right truncate */
...
typedef struct modelRange
{
   ...
   * trait locus and marker. */
  double *dprime;		/* Array of dprime values for LE. */
  int ndprime;			/* Number of dprime values for LE. */
