  ...
  if(modelOptions.mapFlag == SA)
    {
      ...
      w2 = (1.0 - modelOptions.thetaWeight) / (0.5 - modelOptions.thetaCutoff[0]);
	  ...
	  for(j=0; j < num_thetaf-1; j++)
	    {
	      ...
	      avgLR3 = result[(i+1)*num_thetam+j][modelRange.nafreq].het_lr_avg;
	      avgLR4 = result[(i+1)*num_thetam+j+1][modelRange.nafreq].het_lr_avg;
		       ...
		       favg < rf && rf <= thetaf2)
		{
	      ...
	      else if (thetam2 < rm && thetaf2 < rf && rf <= favg)
		{
		  /* case #7 */
		  caseIdx = 7;
		  A = (avgLR1 + avgLR3) * 0.5 * mdiff * (w1 * fdist + w2 * (favg - rf)) +
		    (avgLR2 + avgLR4) * w2 * 0.25 * B;
	      ...
	      else if (thetam2 < rm && favg < rf && rf <= thetaf2)
		{
		  ...
		  A = (avgLR1 + avgLR3) * w1 * 0.25 * B +
		    (avgLR2 + avgLR4) * 0.5 * mdiff * (w1 *(rf - favg) + w2 * (thetaf2 - rf));
		}
	      else if(thetaf2 < rf && thetam1 < rm && rm <= mavg)
		{
		  /* case #9 */
		  caseIdx = 9;
		  A = (avgLR1 + avgLR2) * 0.5 * fdiff * (w1 * mdist + w2 * (mavg - rm)) +
		    (avgLR3 + avgLR4) * w2 * 0.25 * B;
		}
	      else if(thetaf2 < rf && mavg < rm && rm <= thetam2)
		{
		  ...
		  A = (avgLR1 + avgLR2) * w1 * 0.25 * B +
		    (avgLR3 + avgLR4) * 0.5 * fdiff * (w1 *(rm - mavg) + w2 * (thetam2 - rm));
		}
  ...
  for (i = 0; i < modelRange.ntheta; i++)
      ...
      for (j = 0; j < numDPrime; j++)
	{
	  if(isnan(result[j][i][modelRange.nafreq].het_lr_avg))
	    {
	      fprintf(stderr, "Average LR at theta (%f,%f)(%d) and dprime %f(%d) is NAN.\n", 
		      modelRange.theta[0][i],
		      modelRange.theta[1][i],
		      i,
		      pLambdaCell->lambda[j][0][0], j);
		      
	    }
	  else if(isinf(result[j][i][modelRange.nafreq].het_lr_avg))
	    {
	      fprintf(stderr, "Average LR at theta (%f,%f)(%d) and dprime %f(%d) is INF.\n", 
		      modelRange.theta[0][i],
		      modelRange.theta[1][i],
		      i,
		      pLambdaCell->lambda[j][0][0], j);
  ...
  for (dprimeIdx = 0; dprimeIdx < pLambdaCell->ndprime; dprimeIdx++)
      ...
      for (thetaInd = 0; thetaInd < modelRange.ntheta; thetaInd++)
	{
	  ...
	  if (theta <= modelOptions.thetaCutoff[0] + ERROR_MARGIN
		...
		{
		  /* dprime is not 0 and under linkage (theta not 0.5) */
		  le_lr +=
  ...
  for (dprimeIdx = 0; dprimeIdx < pLambdaCell->ndprime; dprimeIdx++)
      ...
      for (thetaInd = 0; thetaInd < modelRange.ntheta; thetaInd++)
	{
	  ...
	  for (mkrFreqIdx = 0; mkrFreqIdx < modelRange.nafreq; mkrFreqIdx++)
		...
		{
		  fprintf(stderr, "het_lr_total is NAN at theta (%f,%f)(%d) and dprime %f(%d).\n",
			  modelRange.theta[0][thetaInd],
			  modelRange.theta[1][thetaInd],
			  thetaInd, 
			  dprime, dprimeIdx);
		}
	      else if(isinf(tp_result[dprimeIdx][thetaInd][mkrFreqIdx].het_lr_total))
		{
		  fprintf(stderr, "het_lr_total is INF at theta (%f,%f)(%d) and dprime %f(%d).\n",
			  modelRange.theta[0][thetaInd],
			  modelRange.theta[1][thetaInd],
			  thetaInd, 
			  dprime, dprimeIdx);
		...
		lr = tp_result[dprimeIdx][thetaInd][mkrFreqIdx].het_lr_total /
		  (tp_result[dprimeIdx][thetaInd][mkrFreqIdx].lr_count/modelRange.ntthresh *
		   2*(modelType.max - modelType.min) * modelRange.nalpha 
		   );
