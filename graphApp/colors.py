...
This module is taken from the MPlot library
...
def mpl_color(c, default = (242,243,244)):
    """Given an mpl color, like 'b' or 'r', return the rgb 
    equivalent as a tuple.
    
    """

    # note: this function is copied from the MPlot library
    try:
...
def hexcolor(color):
    ...
    if type(color) == types.TupleType:
        ...
        # could be an X11 color or matplotlib color, like 'r'
        if len(color) == 1:
            # matplotlib color
            rgb = mpl_color(color)
        else:
...
def getCompliment(c):
    """Given a color as a tuple, wx.Color, or X11 named color, return the 
    complement of that color as a tuple.
    
    """

    # first step is to convert color to rgb tuple.
    # check if it is a hex color
    if type(c) in types.StringTypes and color[0:1] == '#' and len(color)==7: 
        c = hex2rgb(c)
    elif type(c) == types.TupleType:
        # already a tuple, no need for conversion
        pass
    elif type(c) == types.ListType:
        c = tuple(c)
        ...
        if c in x11_colors.keys():  
            rgb = x11_colors[c]
    elif type(c) == wx.Colour:
        c = (color.Red(), color.Green(), color.Blue())
    else:
        # unknown format, just return junk
        print 'Warning! Unknown format in colors.getCompliment()'
        return (0, 0, 0)
