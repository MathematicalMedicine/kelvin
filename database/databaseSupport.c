...
@file databaseSupport.c
 ...
 databaseSupport - support routines for likelihood server studies.

 Written by Bill Valentine-Cooper in a huge rush. Please be kind.
 ...
 The likelihood server started as yet another mechanism for splitting
 analyses into components that can be evaluated in parallel so as to
 reduce total time-to-completion. It grew almost immediately to
 include the ability to combine different types of likelihood
 calcuations (different marker counts for the internal Elston-Stewart
 multipoint calculation, and external likelihood calculation,
 e.g. Merlin's Lander-Green), as well as merge the different study
 maps associated with pedigrees into a single reference map.

 It was written as a shim to be slid in-between dcuhre-controlled
 iteration and likelihood calculation, with minimal modification of
 Kelvin. The basic approach is to define client and server modes of
 operation for Kelvin and alternate running them until the analysis
 completes.

 The client mode of kelvin runs in a completely normal manner up to
 the boundary between dcuhre and likelihood calculation.  At that
 point, it uses the trait model parameters generated by dcuhre as keys
 to attempt to retrieve the corresponding previously-calculated
 likelihood from a database, or, if none is present, store them for
 later evaluation and return a dummy likelihood to dcuhre instead,
 which permits continued processing. If all of the likelihoods
 requested by dcuhre for a region are present in the database, then
 dcuhre is allowed to do error analysis and split the region as
 needed, possibly laying-down new trait models which will themselves
 either be retrieved from the database or stored for later
 evaluation. If any of the likelihoods requested by dcuhre for a
 region are dummy values, then no error analysis is permitted (because
 it would be spurious), and a flag is set to indicate that although
 the analysis seemed to run to completion, the results are
 meaningless. At the end of the first run of the client, the database
 will contain requests for likelihoods for every trait model required
 by dcuhre for the initial region for every position and pedigree. At
 this point, a server-mode run must be performed.

 In server mode, kelvin sets-up normally, but instead of looping over
 specific trait positions specified by the configuration file, it
 loops over a calculated set of positions that will cause Kelvin to
 construct all possible combinations of trait and markers across the
 full analysis position range. As a part of this calculation, it
 determines the low and high positions of the range for which each
 combination of trait and markers is applicable, and then when it
 descends to the level of likelihood calculation, it queries the
 database for all pending models in that range for the current
 pedigree and marker count. The intent is to allow Kelvin to do all of
 the normal setup for performing a representative likelihood
 calculation, and then use that setup to perform all of the related
 likelihood calculations that are desired. As each likelihood
 calculation is performed, the results are stored in the database with
 the corresponding trait model for the client to retrieve later.

 If the full analysis employs different types of likelihood
 calculation, e.g.  2 and 3 marker multipoint, or two-point, then
 different server configurations are required.

 After all applicable servers have run and therefore all trait models
 in the database have corresponding calculated likelihoods, then the
 client is run again. This time, the the database contains actual
 likelihoods for all of the models in the first tier of dcuhre regions,
 so error analysis is performed at all positions and if splits occur,
 new sets of models are requested and a second server run is needed.

 This alternation of client and server runs will be performed to the
 final depth of dcuhre splits. When no more splits are needed, the
 final client run uses the likelihoods from the database to generate
 all of the normal Kelvin results and reports.
 ...
 Uses MySQL 5.1 C API calls, and therefore would be difficult to
 ...
 This code represents the shortest path to completion of our objectives
 in the time available, and has been affected by changing scope within
 that timeframe. Treat it with the caution (and consideration) that you
 would for code you wrote in two 40-hour weekends. Hopefully it will
 evolve into something more straightforward and versatile.
...
/** Global structure containing everything pertinent to a single database connection
    and all of the state information needed to run as a client or server. */
struct StudyDB studyDB; 

/** Macros for preparing parameter structures for MySQL database calls.  Don't 
    use these willy-nilly since they do allocate space for length, is_null and 
    error, and no effort is made to free this space at any time. */
...
void initializeDB () {
  ...
  *studyDB.bindGetStudyId[2].length = strlen(studyDB.imprintingFlag);
  ...
  while(1) {
	...
	swLogProgress(5, 0, "Retry #%d of presumed deadlock after %d second in GetStudyId", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
...
void GetAnalysisId () {
  ...
  *studyDB.bindGetAnalysisId[2].length = strlen(studyDB.pedigreeNotRegEx);
  ...
  while(1) {
	...
	if (++retries > MAX_DB_RETRIES)
	  ERROR("Database operation exceeded %d retries", MAX_DB_RETRIES);
	swLogProgress(5, 0, "Retry #%d of presumed deadlock after %d second in GetAnalysisId", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
...
double GetMarkerSetLikelihood(int pedPosId, int regionNo, int parentRegionNo, double parentRegionError, int parentRegionSplitDir)
{
  ...
  while (1) {
	...
	if (++retries > MAX_DB_RETRIES)
	  ERROR("Database operation exceeded %d retries", MAX_DB_RETRIES);
	swLogProgress(5, 0, "Retry #%d of presumed deadlock after %d second in GetMarkerSetLikelihood", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
		...
		int regionNo, int parentRegionNo, double parentRegionError, int parentRegionSplitDir)
{
  ...
  while (1) {
	...
	if (++retries > MAX_DB_RETRIES)
	  ERROR("Database operation exceeded %d retries", MAX_DB_RETRIES);
	swLogProgress(5, 0, "Retry #%d of presumed deadlock after %d second in GetDLikelihood", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
		       ...
		       int regionNo, int parentRegionNo, double parentRegionError, int parentRegionSplitDir)
{
  ...
  while (1) {
	...
	if (++retries > MAX_DB_RETRIES)
	  ERROR("Database operation exceeded %d retries", MAX_DB_RETRIES);
	swLogProgress(5, 0, "Retry #%d of presumed deadlock after %d second in GetQLikelihood", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
...
int CountWork (double lowPosition, double highPosition)
{
  ...
  while (1) {
	...
	if (++retries > MAX_DB_RETRIES)
	  ERROR("Database operation exceeded %d retries", MAX_DB_RETRIES);
	swLogProgress(5, 0, "Retry #%d of presumed deadlock after %d second in CountWork", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
	      ...
	      double *lC3BigPen, double *lC3BigLittlePen, double *lC3LittleBigPen, double *lC3LittlePen)
{
  ...
  while (1) {
	...
	if (++retries > MAX_DB_RETRIES)
	  ERROR("Database operation exceeded %d retries", MAX_DB_RETRIES);
	swLogProgress(5, 0, "Retry #%d of presumed deadlock after %d second in GetDWork", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
	      ...
	      )
{
  ...
  while (1) {
	...
	if (++retries > MAX_DB_RETRIES)
	  ERROR("Database operation exceeded %d retries", MAX_DB_RETRIES);
	swLogProgress(5, 0, "Retry #%d of presumed deadlock after %d second in GetQWork", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
...
void PutWork (int markerCount, double lOD, int runtimeCostSec)
{
  ...
  */
  ...
  while (1) {
	...
	if (++retries > MAX_DB_RETRIES)
	  ERROR("Database operation exceeded %d retries", MAX_DB_RETRIES);
	swLogProgress(5, 0, "Retry #%d of presumed deadlock after %d second in PutWork", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
