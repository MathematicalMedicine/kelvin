...
void initializeDB () {
  ...
  *studyDB.bindGetStudyId[2].length = strlen(studyDB.imprintingFlag);
  ...
  while(1) {
	...
	INFO("Retry #%d of failed query after %d second in GetStudyId", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
...
void GetAnalysisId () {
  ...
  *studyDB.bindGetAnalysisId[2].length = strlen(studyDB.pedigreeNotRegEx);
  ...
  while(1) {
	...
	if (++retries > MAX_DB_RETRIES)
	  ERROR("Database operation exceeded %d retries", MAX_DB_RETRIES);
	INFO("Retry #%d of failed query after %d second in GetAnalysisId", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
...
double GetMarkerSetLikelihood(int pedPosId, int regionNo, int parentRegionNo, double parentRegionError, int parentRegionSplitDir)
{
  ...
  while (1) {
	...
	if (++retries > MAX_DB_RETRIES)
	  ERROR("Database operation exceeded %d retries", MAX_DB_RETRIES);
	INFO("Retry #%d of failed query after %d second in GetMarkerSetLikelihood", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
		...
		int regionNo, int parentRegionNo, double parentRegionError, int parentRegionSplitDir)
{
  ...
  while (1) {
	...
	if (++retries > MAX_DB_RETRIES)
	  ERROR("Database operation exceeded %d retries", MAX_DB_RETRIES);
	INFO("Retry #%d of failed query after %d second in GetDLikelihood", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
		       ...
		       int regionNo, int parentRegionNo, double parentRegionError, int parentRegionSplitDir)
{
  ...
  while (1) {
	...
	if (++retries > MAX_DB_RETRIES)
	  ERROR("Database operation exceeded %d retries", MAX_DB_RETRIES);
	INFO("Retry #%d of failed query after %d second in GetQLikelihood", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
...
int CountWork (double lowPosition, double highPosition)
{
  ...
  while (1) {
	...
	if (++retries > MAX_DB_RETRIES)
	  ERROR("Database operation exceeded %d retries", MAX_DB_RETRIES);
	INFO("Retry #%d of failed query after %d second in CountWork", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
	      ...
	      double *lC3BigPen, double *lC3BigLittlePen, double *lC3LittleBigPen, double *lC3LittlePen)
{
  ...
  while (1) {
	...
	if (++retries > MAX_DB_RETRIES)
	  ERROR("Database operation exceeded %d retries", MAX_DB_RETRIES);
	INFO("Retry #%d of failed query after %d second in GetDWork", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
	      ...
	      )
{
  ...
  while (1) {
	...
	if (++retries > MAX_DB_RETRIES)
	  ERROR("Database operation exceeded %d retries", MAX_DB_RETRIES);
	INFO("Retry #%d of failed query after %d second in GetQWork", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
...
void PutWork (int markerCount, double lOD, int runtimeCostSec)
{
  ...
  */
  ...
  while (1) {
	...
	if (++retries > MAX_DB_RETRIES)
	  ERROR("Database operation exceeded %d retries", MAX_DB_RETRIES);
	INFO("Retry #%d of failed query after %d second in PutWork", retries, dBRetryDelays[retries]);
	sleep(dBRetryDelays[retries]);
	continue;
      }
    }
