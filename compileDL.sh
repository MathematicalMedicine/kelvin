...
touch ${name}.linking
gcc -g -O${optLevel} -L${KELVIN_ROOT}/lib/ -shared -o ${name}.so ${name}.o >& ${name}-link.out
...
for dl in ${name}_[0-9]*00.o ; do [ -f $dl ] || continue ;
    dl=${dl##*/}
    dl=${dl%00\.o}
    gcc -g -O${optLevel} -L${KELVIN_ROOT}/lib/ -shared -o ${dl}00.so ${dl}[0-9][0-9].o >& ${dl}00-link.out
    if test ! -x ${name}.so ; then
